	...
	class Tonjoo_License_API {
		...
		public function activate( $key = '' ) {
			if ( empty( $key ) ) {
				return array(
					'status'    => false,
					'data'      => __( 'Error: License key is empty', 'plugin-resi' ),
				);
			}

			$url     = $this->server . '/api/license/activate/';
			$args    = wp_parse_args( array(
				'body' => array(
					'code' => $this->encode_parameter( $key )
				)
			), $this->wp_remote_args );
			...
			if ( is_wp_error( $request ) ) {
				...
				return array(
					'status'    => false,
					'data'      => __( "Can't connect to server. Please try again.", 'plugin-resi' ),
				);
			} else {
				if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
					...
					if ( ! $license_status->status ) {
						...
						if ( 'failed activate, the license is already in use' === $license_status->message ) {
							return array(
								'status'    => false,
								'data'      => __( 'Error: The license is already in use. If you sure this license is belongs to you and no other sites are using this license, please contact us at the forum.', 'plugin-resi' ),
							);
						} else {
							return array(
								'status'    => false,
								'data'      => sprintf( __( 'Error: %s. Please try again.', 'plugin-resi' ), $license_status->message ),
							);
						}
					}
				} else {
					if ( 404 === intval( wp_remote_retrieve_response_code( $request ) ) ) {
						// retry.
					...
					return array(
						'status'    => false,
						'data'      => sprintf( __( 'Error: %s. Please contact us through the forum to resolve this.', 'plugin-resi' ), wp_remote_retrieve_response_code( $request ) . ' ' . wp_remote_retrieve_response_message( $request ) ),
					);
				}

				$url        = $this->server . '/api/license/json/?hash=' . $license_status->hash;
				...
				if ( is_wp_error( $request ) ) {
					...
					return array(
						'status'    => false,
						'data'      => sprintf( __( 'Error: %s. Please try again.', 'plugin-resi' ), $request->get_error_message() ),
					);
				} else {
					if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
						// write json file.
		...
		public function deactivate( $key = '' ) {
			...
			$url     = $this->server . '/api/license/deactivate/';
			$args    = wp_parse_args( array(
				'body' => array(
					'code' => rawurlencode( $this->encode_parameter( $key ) )
				)
			), $this->wp_remote_args );
			$request = wp_remote_post( $url, $args );
		...
		public function status( $key = '', $debug = false ) {
			...
			$url        = $this->server . '/api/license/status/?license=' . $key . '&website=' . $this->site;
			$request    = wp_remote_get( $url, $this->wp_remote_args );
			...
			if ( is_wp_error( $request ) ) {
				...
				return array(
					'status'    => false,
					'data'      => __( "Error: Can't connect to server", 'plugin-resi' ),
				);
			} else {
				if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
					...
					if ( isset( $response->error ) && $response->error ) {
						...
						return array(
							'status'    => false,
							'data'      => sprintf( __( 'Error: %s. Please try again.', 'plugin-resi' ), $response->message ),
						);
					}
		...
		public function bulk_status( $keys = array(), $debug = false ) {
			...
			$url = $this->server . '/api/license/bulkstatus/';
		 ...
		 */
		public function update_json( $key = '', $hash = '', $debug = false ) {
			if ( empty( $key ) ) {
				return false;
			}

			$url = $this->server . '/api/license/json/?hash=' . $hash;
