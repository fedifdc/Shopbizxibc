...
function shopbiz_upgrade()
{
  ...
  if (!empty($_FILES['file'])) {
    ...
    // Upload the file using WordPress functions
    $upload_overrides = array('test_form' => false);
    $movefile = wp_handle_upload($uploadedfile, $upload_overrides);
	
    if ($movefile && !isset($movefile['error'])) {
      $foto_ktp_url = $movefile['url'];  // The URL of the uploaded image
	  update_user_meta($userdata->ID,'foto_ktp_url',$foto_ktp_url);
	  update_user_meta($userdata->ID,'foto_type',$file_type);
	  update_user_meta($userdata->ID,'foto_name',$file_name);
	  //update_user_meta($userdata->ID,'foto_tmp',$file_tmp);

    } else {
      $response = array(
        'success' => false,
        'message' => 'Failed to upload KTP image. Please try again.',
      );
