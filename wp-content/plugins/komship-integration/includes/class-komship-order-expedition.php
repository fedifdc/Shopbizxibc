            ...
            jQuery(document).ready(function($) {
                ...
                if (typeof $.fn.selectWoo !== 'undefined') {
                    ...
                    function triggerShippingCalculation() {
                        if ($('#shipper_select').val() && $('#receiver_select').val()) {
                            const kurir   = $('[name="plugin_resi_ekspedisi"]').val();
                            const layanan = $('[name="plugin_resi_layanan"]').val();
                            const data = {
                                action: 'komship_calculate_shipping',
                                nonce: komshipData.nonce,
                                shipper_id: $('#shipper_select').val(),
                                ...
                                if (response.success === true) {
                                    ...
                                    if (shippingOptions.length > 0) {
                                        shippingOptions.forEach(function(shipping) {
                                            ...
                                            if (
                                                shipping.shipping_name.toUpperCase() === kurir.toUpperCase() &&
                                                shipping.service_name.toUpperCase() === layanan.toUpperCase()
                                            ) {
                                                expeditionOptions += `<option value='${shippingObjectString}' selected>
                                                    ${shipping.shipping_name} - ${shipping.service_name} - Rp.${shipping.shipping_cost} ${shipping.is_cod ? "(Bisa COD)" : ""}
                                                </option>`;
                                                 $('#shipping_type').val(layanan);
                                                 $('#shipping_cost').val(shipping.shipping_cost);
                                                 
                                            } else {
                                                expeditionOptions += `<option value='${shippingObjectString}'>
                                                    ${shipping.shipping_name} - ${shipping.service_name} - Rp.${shipping.shipping_cost} ${shipping.is_cod ? "(Bisa COD)" : ""}
                                                </option>`;
                                            }
                                        });
