...
function add_sponsor_settings_menu() {
    add_menu_page(
        'Sponsor Settings',
        'Sponsor Settings',
        'manage_options',
        'sponsor-settings',
        'render_sponsor_settings_page',
        'dashicons-admin-generic',
        90
    );
}
add_action('admin_menu', 'add_sponsor_settings_menu');

// Enqueue custom CSS for modern UI
function sponsor_settings_custom_css() {
    $screen = get_current_screen();
    if ($screen->id === 'toplevel_page_sponsor-settings') {
        echo '
        <style>
            body {
...
add_action('admin_head', 'sponsor_settings_custom_css');

// Render the settings page
function render_sponsor_settings_page() {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['settings_nonce']) && wp_verify_nonce($_POST['settings_nonce'], 'save_settings')) {
        // Badge Settings
        update_option('starter_commission', sanitize_text_field($_POST['starter_commission']));
        update_option('basic_commission', sanitize_text_field($_POST['basic_commission']));
        update_option('premium_commission', sanitize_text_field($_POST['premium_commission']));
        update_option('prestige_commission', sanitize_text_field($_POST['prestige_commission']));
      	update_option('free_member_commission', sanitize_text_field($_POST['free_member_commission']));
      	update_option('reset_user_commission', sanitize_text_field($_POST['reset_user_commission']));
    ...
    // Get settings
    $starter_commission = get_option('starter_commission', 0);
    $basic_commission = get_option('basic_commission', 0);
    $premium_commission = get_option('premium_commission', 0);
    $prestige_commission = get_option('prestige_commission', 0);
   	$free_member_commission = get_option('free_member_commission', 0);
    $commission_data = get_option('sponsor_commission_data', []);
    $reset_user_commission = get_option('reset_user_commission',0);
    ...
    <div class="wrap">
        ...
        <form method="POST" action="">
            ...
            <!-- Badge Settings -->
            <h2>Sponsor Commission for Level 1</h2>
            <table class="form-table">
                ...
                <tr>
                    <th>Reset User Commission (%)</th>
                    <td><input type="number" name="reset_user_commission" value="<?php echo esc_attr($reset_user_commission); ?>" step="0.01" required></td>
                </tr>
            </table>
