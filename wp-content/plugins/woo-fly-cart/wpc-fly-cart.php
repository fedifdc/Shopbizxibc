		...
		if ( ! class_exists( 'WPCleverWoofc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoofc {
				...
				function ajax_update_qty() {
					if ( ! apply_filters( 'woofc_disable_security_check', false, 'update_qty' ) ) {
...
if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
    ...
    foreach ( $items as $cart_item_key => $cart_item ) {
        if ( ! isset( $cart_item['bundled_by'] ) && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
            ...
            // Dapatkan ID vendor dari produk
            ...
            $vendor_items[$vendor_id][$cart_item_key] = $cart_item;
        }
    }
    
    // Loop melalui setiap vendor
    foreach ( $vendor_items as $vendor_id => $vendor_cart_items ) {
        // Dapatkan info vendor
        $vendor = dokan()->vendor->get( $vendor_id );
        $vendor_name = $vendor->get_shop_name();
        $vendor_url = $vendor->get_shop_url();
        
        // Tampilkan header vendor
        echo '<div class="woofc-vendor-group">';
        echo '<div class="woofc-vendor-header">';
        echo '<span class="woofc-vendor-name">' . esc_html( $vendor_name ) . '</span>';
        echo '<a href="' . esc_url( $vendor_url ) . '" target="_blank" class="woofc-vendor-link">' . esc_html__( 'Lihat Toko', 'textdomain' ) . '</a>';
        echo '</div>';
        echo '<div class="woofc-vendor-items">';
        
        // Loop melalui item vendor
        ...
        echo '</div><!-- /.woofc-vendor-items -->';
        echo '</div><!-- /.woofc-vendor-group -->';
    }
} else {
