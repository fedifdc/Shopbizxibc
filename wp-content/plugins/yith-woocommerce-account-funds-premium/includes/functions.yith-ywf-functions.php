...
/**
 * This file mange the functions
 *
 * @package YITH\AccountFunds\Functions
 */
...
if ( ! function_exists( 'ywf_enable_discount' ) ) {
	...
	function ywf_enable_discount() {
		...
		return apply_filters( 'ywf_enable_discount', 'yes' === $enable_discount );
...
if ( ! function_exists( 'ywf_order_has_deposit' ) ) {
	...
	function ywf_order_has_deposit( $order ) {
		$has_deposit = 'yes' === $order->get_meta( '_order_has_deposit' );

		return apply_filters( 'ywf_order_has_deposit', $has_deposit, $order );
...
if ( ! function_exists( 'ywf_get_customize_my_account_menu' ) ) {
	/**
	 * Integration with customize my account page
	 *
	 * @return string
	 * @since 1.0.0
	 */
	function ywf_get_customize_my_account_menu() {
		...
		$tab      = 'tab' === get_option( 'yith-wcmap-menu-style', 'sidebar' ) ? '-tab' : '';
		$menu     = '<div id="my-account-menu' . esc_attr( $tab ) . '" class="yith-wcmap position-' . esc_attr( $position ) . '">' . YITH_WCMAP_Frontend()->my_account_menu() . '</div>';
...
if ( ! function_exists( 'ywf_is_make_deposit' ) ) {
	/**
	 * Check if is make a deposit endpoint
	 *
	 * @return bool
	 * @since 1.0.0
	 */
...
if ( ! function_exists( 'ywf_get_gateway' ) ) {
	...
	function ywf_get_gateway() {
		...
		foreach ( $payment as $gateway ) {
			if ( 'yes' === $gateway->enabled && 'yith_funds' !== $gateway->id ) {
...
if ( ! function_exists( 'ywf_check_if_can_send_email' ) ) {
	...
	/**
	 * Check if is possible send the email
	 *
	 * @param bool $send_email Can send email.
	 * @param int  $user_id The user id.
	 *
	 * @return bool
	 * @since 1.0.0
...
if ( ! function_exists( 'ywf_update_db_1_0_1' ) ) {
	/**
	 * Update the db to version 1.0.1
	 *
	 * @since 1.0.1
	 */
...
if ( ! function_exists( 'yith_account_funds_clear_session' ) ) {
	...
	function yith_account_funds_clear_session() {
		...
		if ( ! is_null( WC()->session ) ) {
			...
			delete_transient('ywf_partial_payment');
			delete_transient('ywf_fund_used');
...
if ( ! function_exists( 'yith_account_funds_get_endpoint_slug' ) ) {
	/**
	 * Get the endpoint slug
	 *
	 * @param string $endpoint The endpoint.
	 *
	 * @return string the right endpoint slug
	 */
	function yith_account_funds_get_endpoint_slug( $endpoint ) {
		...
		$slug                   = '';
		if ( in_array( $endpoint, $account_funds_endpoint, true ) ) {
