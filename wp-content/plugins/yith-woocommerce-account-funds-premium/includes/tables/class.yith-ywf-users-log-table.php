...
/**
 * Class that manage the table that show the user log list
 *
...
if ( ! class_exists( 'YITH_YWF_Users_Log_Table' ) ) {
	/**
	 * Class YITH_YWF_Users_Log_Table
	 */
	class YITH_YWF_Users_Log_Table extends WP_List_Table {
		...
		/**
		 * YITH_YWF_Users_Log_Table constructor.
		 *
		...
		public function column_default( $item, $column_name ) {
			...
			switch ( $column_name ) {
				...
				case 'desc':
					...
					break;
				case 'date_mov':
					$date         = gmdate( get_option( 'date_format' ), strtotime( $item->date_added ) );
		...
		public function sort_by( $a, $b ) {
			...
			$orderby = ( ! empty( $_REQUEST['orderby'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['orderby'] ) ) : 'date_mov'; // phpcs:ignore WordPress.Security.NonceVerification
			$order   = ( ! empty( $_REQUEST['order'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['order'] ) ) : 'desc'; // phpcs:ignore WordPress.Security.NonceVerification

			if ( 'user_name' === $orderby ) {
