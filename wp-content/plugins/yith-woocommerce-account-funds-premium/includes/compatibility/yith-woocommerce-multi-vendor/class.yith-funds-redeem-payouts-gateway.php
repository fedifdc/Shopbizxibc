...
/**
 * Class that manage the compatibility with YITH Multi Vendor Plugin : Payouts Gateway
 *
...
class YITH_Redeem_Funds_Payouts_Gateway {
	 ...
	 *
	 * @var YITH_Redeem_Funds_Payouts_Gateway
	 */
	protected static $instance;

	/**
	 * YITH_Redeem_Funds_Payouts_Gateway constructor.
	 */
	 ...
	 *
	 * @return YITH_Redeem_Funds_Payouts_Gateway
	 * @since 1.4.0
	...
	public function redeem( $vendor_data ) {
		...
		$batch = YITH_PayOuts_Service()->PayOuts(
			array(
				'sender_batch_id' => $sender_batch_id,
	 ...
	 *
	 * @param string $payout_item_id The payout item id.
	 * @param string $transaction_status The transaction status.
		...
		if ( $sender_item_id ) {
			...
			switch ( $transaction_status ) {
				...
				case 'pending':
					...
					if ( ! in_array( $old_status, $check_status, true ) ) {
						$customer = new YITH_YWF_Customer( $sender_item_id );

						$fund_log_args = array(
							'type_operation' => 'redeem_vendor_funds',
							'description'    => sprintf(/* translators: %s is the amount of redeem */
								__( 'Redeem %s funds with PayPal Payouts', 'yith-woocommerce-account-funds' ),
				...
				case 'blocked':
					...
					$fund_log_args = array(
						...
						'description'    => sprintf( __( 'Restored %1$s funds because payout %2$s is %3$s', 'yith-woocommerce-account-funds' ), wc_price( $amount, array( 'currency' => $currency ) ), $transaction_id, $transaction_status ),
					);
...
if ( ! function_exists( 'YITH_Redeem_Funds_with_Payouts' ) ) {
	 ...
	 * @return YITH_Redeem_Funds_Payouts_Gateway
	 */
	function YITH_Redeem_Funds_with_Payouts() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
