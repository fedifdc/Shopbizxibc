...
/**
 * Class that manage the compatibility with YITH Multi Vendor Plugin
 *
...
if ( ! class_exists( 'YITH_FUNDS_MultiVendor' ) ) {
	...
	class YITH_FUNDS_MultiVendor {
		/**
		 * YITH_FUNDS_MultiVendor constructor.
		 */
		public function __construct() {
			...
			$payment_method_enabled = ( ( $this->is_paypal_payouts_enabled() && $this->is_paypal_payouts_method() ) || ( $this->is_stripe_connect_enabled() && $this->is_stripe_connect_method() ) );
			if ( defined( 'YITH_WPV_PREMIUM' ) && defined( 'YITH_WPV_VERSION' ) && version_compare( YITH_WPV_VERSION, '3.7.0', '>=' ) && ( 'yes' === $vendor_can_redeem && $payment_method_enabled ) ) {
				...
				$manual_redeem = get_option( 'ywf_enable_manual_redeeming', 'no' );
				if ( yith_plugin_fw_is_true( $manual_redeem ) ) {
					add_action( 'woocommerce_account_' . $redeem_slug . '_endpoint', array( $this, 'show_redeem_funds_endpoint' ) );
					add_filter( 'woocommerce_endpoint_' . $redeem_slug . '_title', array( $this, 'show_redeem_funds_endpoint_title' ) );
				...
				if ( $this->is_paypal_payouts_method() ) {
					add_filter( 'yith_payouts_payment_mode', array( $this, 'add_payouts_redeem_funds_payment_mode' ), 15, 1 );
					add_filter( 'yith_paypal_payout_redeem_funds_items_html', array( $this, 'get_redeem_funds_items_html' ), 10, 4 );
				...
				add_action( 'update_option_ywf_redeeming_payment_type', array( $this, 'redeem_funds_delete_schedule' ) );
		...
		/**
		 * Add funds information in suborders
		 *
		...
		/**
		 * Show the fund information in suborder
		 *
		 * @param bool     $show Can show the fund info.
		 * @param string   $is_partial Is partial payment.
		...
		public function show_fund_editor_metabox( $show, $is_partial, $order ) {
			...
			if ( $vendor->is_valid() && $vendor->has_limited_access() ) {
				if ( 0 !== intval( wp_get_post_parent_id( $order_id ) ) ) {
		...
		/**
		 * Add in the commission , custom operations
		 *
		 * @param array $operations The old commission operations.
		 *
		 * @return array
		 */
		...
		/**
		 * Add in my account an endpoint to redeem vendor funds
		 *
		...
		public function add_redeem_endpoint( $menu_items ) {
			...
			if ( yith_plugin_fw_is_true( $manual_redeem ) && $vendor->is_valid() && $vendor->is_owner( $current_user_id ) ) {
		...
		/**
		 * Show the redeem funds template
		 *
		 * @since 1.4.0
		 */
		...
		/**
		 * Add custom css in the frontend
		 *
		 * @since 1.4.0
		 */
		public function include_button_style() {

			$text_color       = get_option(
			...
			wp_enqueue_script( 'ywf_redeem_funds', YITH_FUNDS_ASSETS_URL . 'js/' . yit_load_js_file( 'ywf-redeem-funds.js' ), array( 'jquery' ), YITH_FUNDS_VERSION, true );
		...
		/**
		 * Detect the redeem funds action
		 *
		 * @since 1.4.0
		 */
		public function redeem_funds_handler() {

			if ( isset( $_POST['yith_redeem_funds'] ) ) { // phpcs:ignore

				$redeem_amount = $_POST['yith_redeem_funds']; // phpcs:ignore
		...
		/**
		 * Detect the redeem funds ajax action
		 *
		 * @since 1.4.0
		 */
		public function redeem_funds_ajax() {

			if ( isset( $_REQUEST['yith_redeem_funds'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$redeem_amount = wp_unslash( $_REQUEST['yith_redeem_funds'] );  // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$this->redeem_funds( $redeem_amount );
				wp_die( 0 );
			}
		}
		 ...
		 * Redeem funds
		 *
		 * @param float $redeem_amount The redeem amount.
		...
		public function redeem( $redeem_amount, $user_id ) {
			...
			try {
				$vendor = function_exists( 'yith_wcmv_get_vendor' ) ? yith_wcmv_get_vendor( $user_id, 'user' ) : yith_get_vendor( 'current', 'user' );
		...
		protected function send_ajax_failure_response() {
			$is_ajax = function_exists( 'wp_doing_ajax' ) ? wp_doing_ajax() : is_ajax();
			if ( $is_ajax ) {
		...
		/**
		 * Delete the current redeem cron
		 *
		 * @since 1.4.0
		 */
		...
		/**
		 * Process the monthly redeem funds action
		 *
		 * @throws Exception The exception.
		 * @since  1.4.0
		 */
		public function redeem_funds_month_cron() {
			...
			if ( 'automatic_with_date' === $automatic_redeem_mode ) {
				...
				$today               = gmdate( 'j' ); // get the current day.
				$how_days_this_month = gmdate( 't' );// get the number days of current month.
		...
		public function is_paypal_payouts_enabled() {
			...
			return defined( 'YITH_PAYOUTS_PREMIUM' ) && version_compare( YITH_PAYOUTS_VERSION, '1.0.12', '>=' );
		}
		...
		public function is_stripe_connect_enabled() {
			return false && defined( 'YITH_WCSC_PREMIUM' );
		}

		/**
		 * Add in the payouts, the redeem funds as payment mode
		 *
		 * @param array $payment_mode The default payment mode.
		 *
		 * @return array
		 * @since 1.4.0
		 */
		...
		/**
		 * Return as string the redeem funds details for the payout table
		 *
		 * @param string $items The items.
		 * @param string $sender_batch_id Payout sender batch id.
		 * @param array  $payout The payout.
		 * @param array  $payout_item The item.
		 *
		 * @return string
		 * @since 1.4.0
		 */
		...
		/**
		 * Add in the Payouts table a custom column
		 *
		 * @param array $columns The list column.
		 *
		 * @return array
		 * @since 1.4.0
		 */
		...
		/**
		 * Remove from MyAccount menu the  make a deposit item
		 *
		...
		/**
		 * Check if a vendor can deposit funds
		 *
		 * @return bool
		 * @since 1.4.0
		 */
		...
		/**
		 * Check if the vendor can use funds
		 *
		 * @param bool $can_use True or false.
		 *
		 * @return bool
		 * @since 1.4.0
		 */
		...
		/**
		 * Check if send or not the charge funds email to vendors
		 *
		 * @param bool    $is_enabled If email is enabled.
		...
		public function enable_charge_funds_email_for_vendor( $is_enabled, $user ) {
			...
			if ( $user instanceof WP_User ) {
				$vendor = function_exists( 'yith_wcmv_get_vendor' ) ? yith_wcmv_get_vendor( $user->ID, 'user' ) : yith_get_vendor( 'current', 'user' );
		 ...
		 *
		 * @param bool $is_purchasable If is purchasable.
		 *
		 * @return bool
		 * @since 1.4.0
		 */
