...
/**
 * Class that manage the compatibility with WPML
 *
...
if ( ! class_exists( 'YITH_FUNDS_WCML' ) ) {
	...
	class YITH_FUNDS_WCML {
		/**
		 * YITH_FUNDS_WCML constructor.
		 */
		public function __construct() {

			add_filter( 'yith_show_available_funds', array( $this, 'convert_base_currency_amount_to_user_currency' ), 10 );
		...
		/**
		 * Convert the discount amount if is fixed
		 *
		 * @param int|float $discount The discount.
		...
		/**
		 * Convert the fund amount in the default currency
		 *
		 * @param float $value The amount to add in customer wallet.
		...
		/**
		 * Show in the order details the right value
		 *
		 * @param float $value The funds.
		 * @param int   $order_id The order id.
		 *
		 * @return float
		 */
		 ...
		 * @param bool|string $order_currency The order currency.
		 *
		 * @return float|int
		 */
