...
/**
 * Class that manage the email send to admin when a vendor redeem funds
 *
...
if ( ! class_exists( 'YITH_YWF_Vendor_Redeem_Email' ) ) {
	/**
	 * Class YITH_YWF_Vendor_Redeem_Email
	 */
	class  YITH_YWF_Vendor_Redeem_Email extends WC_Email {

		/**
		 * The redeem transaction list
		 *
		 * @var array
		 */
		...
		/**
		 * YITH_YWF_Vendor_Redeem_Email constructor.
		 */
		...
		public function trigger( $args ) {
			...
			if ( $this->is_enabled() && ! empty( $args ) && $this->get_recipient() ) {
				...
				$this->placeholders['{date}'] = wc_format_datetime( new WC_DateTime( date( 'Y-m-d' ), new DateTimeZone( 'UTC' ) ) ); //phpcs:ignore
		 ...
		 *
		 * @param string $key The key.
		 * @param mixed  $empty_value The empty value.
		 *
		 * @return  mixed
		 * @since   1.4.0
		 */
