<?php
/**
 * Show the funds history for specific user
 *
...
$my_deposit_history = array(
	'description'    => __( 'Description', 'yith-woocommerce-account-funds' ),
	'log-date'       => __( 'Date', 'yith-woocommerce-account-funds' ),
	'deposit-amount' => __( 'Transactions', 'yith-woocommerce-account-funds' ),
);

$fund_totals    = 0;
$filter_type    = isset( $_POST['filter_deposit_type'] ) ? wp_unslash( $_POST['filter_deposit_type'] ) : ''; // phpcs:ignore
...
<div class="ywf_history_container">
		...
		<form method="post">
			<div class="filter_container">
				<select name="filter_deposit_type">
					<option value="" <?php selected( '', $filter_type ); ?>><?php esc_html_e( 'Show All', 'yith-woocommerce-account-funds' ); ?></option>
					<?php foreach ( $operation_type as $key => $type_description ) : ?>
						<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $filter_type, $key ); ?>><?php echo esc_html( $type_description ); ?></option>
					<?php endforeach; ?>
				</select>
				<input type="submit" class="button ywf_button" value="<?php esc_attr_e( 'Filter', 'yith-woocommerce-account-funds' ); ?>">
			</div>
		</form>
	<?php endif; ?>
	<?php if ( count( $user_log_items ) > 0 ) : ?>
		<table class="shop_table shop_table_responsive my_account_orders my_funds_history">
			...
			<tbody>
			<?php
			foreach ( $user_log_items as $log ) :


				$date_deposit = sprintf( 'date-type_operation="%s"', $log->type_operation );
				?>
				<tr class="order" <?php echo esc_attr( $date_deposit ); ?> >
					<?php
					foreach ( $my_deposit_history as $column_id => $column_name ) :
						switch ( $column_id ) {

							case 'description':
								$funds_used = get_post_meta( $log->order_id, '_order_funds', true );

								$order      = wc_get_order( $log->order_id ); // phpcs:ignore
								$order_link = '';
								$item_count = 0;
								if ( $order ) {
							...
							case 'log-date':
								$content = sprintf( '<time datetime="%s" title="%s">%s</time>', gmdate( 'Y-m-d', strtotime( $log->date_added ) ), esc_attr( strtotime( $log->date_added ) ), date_i18n( get_option( 'date_format' ), strtotime( $log->date_added ) ) );
								break;

							case 'deposit-amount':
								$fund_user   = apply_filters( 'yith_show_used_funds', $log->fund_user );
								$fund_totals = $fund_totals + floatval( $fund_user );
			...
			if ( $show_total ) :
				...
				<tr class="fund_totals">
					<td colspan="<?php echo count( $my_deposit_history ) - 1; ?>"><?php esc_html_e( 'Your available funds', 'yith-woocommerce-account-funds' ); ?></td>
					<td colspan="1">
						<?php
						echo wc_price( $total ); //phpcs:ignore WordPress.Security.EscapeOutput
						?>
					</td>
				</tr>
			<?php endif; ?>
			<?php if ( ! empty( $page_links ) ) : ?>
				<tr class="pagination-row">
					<td colspan="<?php echo count( $my_deposit_history ); ?>"><?php echo $page_links; //phpcs:ignore WordPress.Security.EscapeOutput ?></td>
				</tr>
			<?php endif; ?>
			</tbody>
		</table>
	<?php else :
		...
		$error_key = isset( $_POST['filter_deposit_type'] ) ? sanitize_text_field( wp_unslash( $_POST['filter_deposit_type'] ) ) : false; // phpcs:ignore
		$label_error = $error_key && isset( $operation_type[ $error_key ] ) ? strtolower( $operation_type[ $error_key ] ) : '';
		?>
		<span>
			<?php
			/* translators: %s displays "deposit" placeholder label */
			echo esc_html( sprintf( __( 'No %s history for this user', 'yith-woocommerce-account-funds' ), $label_error ) );
			?>
		</span>
	<?php endif; ?>
</div>
