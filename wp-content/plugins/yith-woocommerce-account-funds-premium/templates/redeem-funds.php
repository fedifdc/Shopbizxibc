<?php
/**
 * Template that show the redeem funds form for the vendors
 *
...
if ( $vendor && $vendor->is_valid() && $vendor->is_owner( $current_user_id ) ) :
	...
	<div id="yith_account_funds_redeem_funds_container">
		...
		if ( 'redeem_made' === $query_var ) {
			...
			if ( floatval( 0 ) === floatval( $fund_available ) ) {
				...
				wc_print_notice( sprintf( __( 'You don\'t have enough funds to redeem. Minimum required funds to redeem is %1$s, available funds is %2$s', 'yith-woocommerce-account-funds' ), wc_price( $min_to_redeem, array( 'currency' => $default_currency ) ), wc_price( $fund_available, array( 'currency' => $default_currency ) ) ), 'error' );
			} else {
				?>
				<div id="yith_account_funds_redeem_form">
					<form name="redeem_funds" method="post" action="<?php echo esc_url( wc_get_endpoint_url( yith_account_funds_get_endpoint_slug( 'redeem-funds' ) ) ); ?>" class="yith_redeem_funds_form" enctype="multipart/form-data">
						<label for="yith_redeem_funds"><?php esc_html_e( 'Amount to redeem', 'yith-woocommerce-account-funds' ); ?></label>
						...
						echo sprintf( $price_format, $currency_symbol, $field_number ); //phpcs:ignore WordPress.Security.EscapeOutput
						?>
					</span>
						<?php if ( $lang ) : ?>
						...
						<?php endif; ?>
						<p>
							<input type="submit" class="button" id="yith_redeem_button" value="<?php echo esc_attr( $button_label ); ?>">
						</p>
