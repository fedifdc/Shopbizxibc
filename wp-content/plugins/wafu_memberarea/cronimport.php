...
$import = $wpdb->get_results("SELECT * FROM `wafu_import_mlm` LIMIT 0,".rand($jmlmin,$jmlmax));
$datenow = strtotime(wp_date('Y-m-d H:i:s'));
$show = '';
if (count($import) > 0) {
	foreach ($import as $import) {
		# Cek apakah nomor sudah terdaftar
		$wa = wafu_format_mlm($import->import_wa);
		$member = $wpdb->get_row("SELECT * FROM `wafu_member_mlm` WHERE `member_wa`='".$wa."'");
		if (isset($member->member_id)) {
			$member_id = $member->member_id;
			# Jika ada data nama, update datanya
			if ($import->import_nama != '') {
				$wpdb->query("UPDATE `wafu_member_mlm` SET `member_nama`='".$wpdb->_real_escape($import->import_nama)."'
			...
			$wpdb->query("INSERT INTO `wafu_member_mlm` (`member_nama`,`member_wa`,`member_tglgabung`,`camp_user_wp`) VALUE 
					('".$wpdb->_real_escape($import->import_nama)."','".$import->import_wa."','".wp_date('Y-m-d H:i:s')."',".$import->camp_user_wp.")");
			$member_id = $wpdb->insert_id;
		}

		# Cek apakah member sudah terdaftar
		$gtm = $wpdb->get_row("SELECT * FROM `wafu_gruptomember_mlm` WHERE `member_id`=".$member_id." 
			AND `grup_id`=".$import->import_grup);
		if (!isset($gtm->gtm_id)) {
			$camp = $wpdb->get_row("SELECT * FROM `wafu_campaign_mlm` WHERE `camp_grup`=".$import->import_grup." ORDER BY `camp_sort` LIMIT 0,1");
    		...
    		$wpdb->query("INSERT INTO `wafu_gruptomember_mlm` 
				...
				VALUES (".$member_id.",".$import->import_grup.",".$nextid.",'".$schedule."',0,1,'".$dbcustom."','".wp_date('Y-m-d H:i:s')."',".$import->camp_user_wp.")");
    		$show .= $import->import_wa.' telah dimasukkan<br/>';
		} else {			
			$show .= 'DUPLIKAT: '.$import->import_wa.'<br/>';
		}
		# Hapus dari tabel import
		$wpdb->query("DELETE FROM `wafu_import_mlm` WHERE `import_id`=".$import->import_id);
	}
} else {
