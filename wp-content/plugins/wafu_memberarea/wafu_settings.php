...
if (isset($_POST['service']) && $_POST['service'] != '') {
	...
	update_user_meta($current_user_id, 'wafusetting', $wafusetting);
	echo '
...
if (isset($_POST['nohp']) && $_POST['nohp'] != '') {
  ...
  if (isset($txterror) && $txterror != '') {
  	...
  	$spintax = new wafu_spin_mlm();
		...
		$cek = wafu_kirim_mlm(wafu_format_mlm($_POST['nohp']), $konten,$gambar,$current_user_id);
...
<form action="" method="post">
  ...
  <div class="form-group row">
	...
	- komisi
	*/
	$groups = $wpdb->get_results($wpdb->prepare("SELECT * FROM `wafu_grup_mlm` WHERE `camp_user_wp` = %d ORDER BY `grup_nama`", $current_user_id));
	?>
	<div class="form-group row">
...
<strong>URL Webhook: </strong><code><?php echo plugins_url('wafu_memberarea/wafu_webhook.php');?>?user_id=<?php echo $current_user_id; ?></code></p>
</div>
...
<form action="" method="post" enctype="multipart/form-data">
	...
	<div class="form-group row">
        ...
        <div class="col-sm-10">
            ...
            <img id="blah" src="<?php echo plugins_url('wafu_memberarea/kosong.png');?>" alt="" class="img-fluid" />
        </div>
    </div>
	<div class="form-group row">
