...
if ( ! function_exists( 'bdp_archive_invert_grid_template' ) ) {
	 ...
	 * @param string $alter_val alter value.
	...
	function bdp_archive_invert_grid_template( $bdp_settings, $alter, $prev_year, $alter_val, $paged ) {
		...
		if ( isset( $bdp_settings['blog_unique_design'] ) && '' != $bdp_settings['blog_unique_design'] ) {
			$firstpost_unique_design = $bdp_settings['blog_unique_design'];
		} else {
			$firstpost_unique_design = 0;
		}
		$unique_design_option = isset( $bdp_settings['unique_design_option'] ) ? $bdp_settings['unique_design_option'] : '';

		if ( 1 == $firstpost_unique_design && 'first_post' === $unique_design_option && 1 == $alter && 0 == $prev_year && 1 == $paged ) {
			$post_thumbnail = 'full';
			echo '<div class="invert-grid-wrap first_post">';
		} elseif ( 1 == $firstpost_unique_design && 'featured_posts' === $unique_design_option && $alter <= $count_sticky && 0 == $prev_year && 1 == $paged ) {
			$post_thumbnail = 'full';
			echo '<div class="invert-grid-wrap first_post">';
		} else {
			if ( 1 != $firstpost_unique_design && 1 == $alter ) {
				echo '<div class="invert-grid-wrapper">';
			} elseif ( $paged > 1 && 1 == $alter ) {
				echo '<div class="invert-grid-wrapper">';
			} elseif ( 1 == $firstpost_unique_design && 'first_post' === $unique_design_option && 1 == $paged && 2 == $alter ) {
				echo '<div class="invert-grid-wrapper">';
			} elseif ( 1 == $firstpost_unique_design && 'featured_posts' === $unique_design_option && 1 == $paged ) {
				$count_sticky = $count_sticky++;
				if ( $alter == $count_sticky ) {
					echo '<div class="invert-grid-wrapper">';
				}
			}
		}
		...
		<?php
		if ( 1 == $firstpost_unique_design && 'first_post' === $unique_design_option && 0 == $prev_year && 1 == $paged && 1 == $alter ) {
			echo '</div>';
		} elseif ( 1 == $firstpost_unique_design && 'featured_posts' === $unique_design_option && $alter <= $count_sticky && 0 == $prev_year && 1 == $paged ) {
			echo '</div>';
		} elseif ( 1 == $prev_year && $wp_query->post_count == $alter ) {
			echo '</div>';
		} elseif ( 1 != $firstpost_unique_design && $wp_query->post_count == $alter ) {
			echo '</div>';
		}
	}
}
