...
if ( ! function_exists( 'bdp_archive_overlay_horizontal_template' ) ) {
	...
	function bdp_archive_overlay_horizontal_template( $bdp_settings ) {
		...
		?>
		<div class="blog_template bdp_blog_template overlay_horizontal blog-wrap lb-item" data-id="<?php echo esc_attr( get_the_date( 'd/m/Y' ) ); ?>" data-description="<?php echo esc_attr( get_the_title() ); ?>">
						...
						<div class="photo post-image">
							...
							if ( has_post_thumbnail() ) {
								...
								<a href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>">
									...
									$resize_image = Bdp_Utility::image_resize( $width, $height, $url, true, get_post_thumbnail_id() );
									if ( ! empty( $resized_image['url'] ) && ! empty( $resized_image['width'] ) && ! empty( $resized_image['height'] ) ) {
										echo '<img class="' . esc_attr( $lazyload_class ) . '" src="' . esc_url( $resize_image['url'] ) . '" data-src="' . esc_url( $resize_image['url'] ) . '" width="' . esc_attr( $resize_image['width'] ) . '" height="' . esc_attr( $resize_image['height'] ) . '" title="' . esc_attr( $post->post_title ) . '" alt="' . esc_attr( $post->post_title ) . '" />';
									}
									?>
								</a>
								<?php
								...
								<a href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>">
									...
									$width        = isset( $bdp_settings['item_width'] ) ? $bdp_settings['item_width'] : 400;
									$height       = isset( $bdp_settings['item_height'] ) ? $bdp_settings['item_height'] : 570;
									$resize_image = Bdp_Utility::image_resize( $width, $height, $url, true, $bdp_settings['bdp_default_image_id'] );
