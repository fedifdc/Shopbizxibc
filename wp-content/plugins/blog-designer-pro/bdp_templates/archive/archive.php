...
<div id="primary" class="content-area">
	<main id="main" class="site-main" role="main">
		...
		if ( 'crayon_slider' === $bdp_theme || 'sunshiny_slider' === $bdp_theme || 'sallet_slider' === $bdp_theme || 'colorful_sliding' === $bdp_theme || 'blog_carousel' === $bdp_theme || 'leest' === $bdp_theme ) {
			$unique_id = wp_rand();
		}
		...
		<div class="blog_template bdp_wrapper <?php echo esc_attr( $bdp_theme ); ?>_cover bdp_archive <?php echo esc_attr( $bdp_theme ) . ' layout_id_' . esc_attr( $archive_id ); ?> ">
			...
			if ( 'leest' === $bdp_theme ) {
				...
				if ( 'top' === $leest_design ) {
					echo '<div class="main-leest-class ' . esc_html( $leest_design ) . '">';
			...
			if ( $loop->have_posts() ) {
				while ( $loop->have_posts() ) :
					...
					if ( 'timeline' === $bdp_theme && ( 'date' === $orderby || 'modified' === $orderby ) ) {
						$prev_year = $prev_year;
					} else {
						$prev_year = 0;
					}
			...
			$slider_array = array( 'cool_horizontal', 'overlay_horizontal', 'crayon_slider', 'sunshiny_slider', 'sallet_slider', 'colorful_sliding', 'blog_carousel', 'leest', 'threed_carousel', 'flip_book_3d' );
