 ...
 * This template can be overridden by copying it to yourtheme/bdp_templates/blog/banner.php.
 *
...
?>
<div class="bdp_blog_template banner-post post-body-div col-lg-3 col-sm-6 <?php echo esc_attr( $category ); ?>">
	<?php
	...
	<div class="post-body-div-inner">
		...
		if ( Bdp_Utility::get_first_embed_media( $post->ID, $bdp_settings ) && 1 == $bdp_settings['rss_use_excerpt'] ) {
			...
			$class = ( 'video' === get_post_format() ) ? 'banner-img-wrapper post-video bdp-video' : 'banner-img-wrapper bdp-post-image post-video';
			echo '<div class="' . esc_attr( $class ) . '">';
			echo wp_kses( Bdp_Utility::get_first_embed_media( $post->ID, $bdp_settings ), Bdp_Admin_Functions::args_kses() );
			echo '</div>';
		} else {
			$post_thumbnail      = 'banner_thumb';
			...
			if ( ! empty( $thumbnail ) ) {
				echo '<div class="banner-img-wrapper bdp-post-image">';
		...
		<div class="bdp_post_content <?php echo ( $has_thumbnail ) ? 'no-thumbnail' : ''; ?>">
			...
			if ( 'post' === $bdp_settings['custom_post_type'] ) {
				if ( isset( $bdp_settings['display_category'] ) && ( 1 == $bdp_settings['display_category'] || 1 == $bdp_settings['display_date'] || 1 == $bdp_settings['display_author'] || 1 == $bdp_settings['display_postlike'] || 1 == $bdp_settings['display_comment_count'] ) ) {
					?>
						...
						if ( in_array( $post_type, $bdp_all_post_type ) ) {
								...
								if ( isset( $bdp_settings[ 'display_taxonomy_' . $taxonomy ] ) && 1 == $bdp_settings[ 'display_taxonomy_' . $taxonomy ] ) {
									...
									if ( isset( $taxonomy ) && ! in_array( $taxonomy, $bdp_exclude_taxonomy ) ) {
										if ( isset( $term_list ) && ! empty( $term_list ) ) {
											?>
										<div class="bdp-banner-tag <?php echo ( $taxonomy_link ) ? 'bdp-has-links' : 'bdp-no-links'; ?>">
							...
							if ( isset( $bdp_settings['display_tag'] ) && 1 == $bdp_settings['display_tag'] ) {
								...
								if ( $tags_list ) {
									...
									echo '<div class="bdp-banner-tag ' . esc_attr( $class ) . '">';
									?>
