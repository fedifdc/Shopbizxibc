...
bdp_blog_single_post_wrapp <?php echo esc_attr( $category ); ?>">
	...
	if ( has_post_thumbnail() || ( isset( $bdp_settings['bdp_default_image_id'] ) && '' != $bdp_settings['bdp_default_image_id'] ) ) {
		...
		} elseif ( 'product' === $bdp_settings['custom_post_type'] ) {
			if ( isset( $bdp_settings['display_taxonomy_product_cat'] ) && 1 == $bdp_settings['display_taxonomy_product_cat'] ) {
				$categories_link    = ( isset( $bdp_settings['disable_link_taxonomy_product_cat'] ) && 1 == $bdp_settings['disable_link_taxonomy_product_cat'] ) ? false : true;
	...
	if ( ( has_post_thumbnail() || ( isset( $bdp_settings['bdp_default_image_id'] ) && '' != $bdp_settings['bdp_default_image_id'] ) ) && 'post' === $bdp_settings['custom_post_type'] ) {
				...
				if ( 'post' !== $bdp_settings['custom_post_type'] ) {
					...
					foreach ( $taxonomy_names as $taxonomy_single ) {
						...
						if ( isset( $bdp_settings[ 'display_taxonomy_' . $taxonomy ] ) && 1 == $bdp_settings[ 'display_taxonomy_' . $taxonomy ] ) {
							...
							if ( isset( $taxonomy ) && 'product_cat' !== $taxonomy ) {
