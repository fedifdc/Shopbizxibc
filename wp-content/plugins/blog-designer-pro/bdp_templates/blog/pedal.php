 ...
 * This template can be overridden by copying it to yourtheme/bdp_templates/blog/pedal.php.
 *
...
<div class="bdp_blog_template blog_template pedal_blog bdp_blog_single_post_wrapp <?php echo esc_attr( $category ); ?>">
	...
	?>
	<div class="bdp-post-image <?php echo ( empty( $thumbnail ) ) ? 'bdp-no-thumbnail' : 'bdp_image_thumbnail'; ?>">
		<div class="bdp_date_author_div">
			<?php
			if ( 1 == $display_date ) {
				...
				echo '<div class="post-date ' . esc_attr( $date_class ) . '">';
				...
				echo wp_kses( substr( $bdp_date, 0, 3 ), Bdp_Admin_Functions::args_kses() );
				echo '</div>';
				echo ( $date_link ) ? '</a>' : '';
				echo '</div>';
			}
	...
	if ( 2 == $read_more_on && 1 == $read_more_link && 1 == $bdp_settings['rss_use_excerpt'] ) {
		...
		echo '<a class="more-tag" href="' . esc_url( $post_link ) . '" target="' . esc_html( $link_behaviour ) . '"> ' . esc_html( $readmoretxt ) . '&nbsp;&nbsp;<i class="fas fa-long-arrow-alt-right"></i></a>';
		echo '</div>';
	}
