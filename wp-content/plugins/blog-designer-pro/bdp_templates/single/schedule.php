 ...
 * This template can be overridden by copying it to yourtheme/bdp_templates/single/schedule.php.
 *
...
add_action( 'bd_single_design_format_function', 'bdp_single_schedule_template', 10, 1 );

if ( ! function_exists( 'bdp_single_schedule_template' ) ) {

	/**
	 * Add html for schedule template
	 *
	 * @global object $post
	 * @param array $bdp_settings settings.
	 * @return void
	 */
	function bdp_single_schedule_template( $bdp_settings ) {
		...
		?>
		<div class="blog_template bdp_blog_template schedule">
		...
		<div class="bdp-post-meta">
			...
			if ( in_array( $post_type, $bdp_all_post_type ) ) {
				...
				if ( 'product' === $post_type ) {
					$bdp_tax_cat = 'product_tag';
				} elseif ( 'download' === $post_type ) {
					$bdp_tax_cat = 'download_tag';
				}
				if ( '' != $bdp_tax_cat && isset( $bdp_settings[ 'display_taxonomy_' . $bdp_tax_cat ] ) && 1 == $bdp_settings[ 'display_taxonomy_' . $bdp_tax_cat ] ) {
					...
					$sep                = 1;
					?>
					<div class="tags <?php echo ( $categories_link ) ? 'bdp-no-links' : 'bdp-has-links'; ?>">
						<?php
		...
		<div class="meta-comment">
			<?php
			if ( isset( $bdp_settings['display_comment_count'] ) && 1 == $bdp_settings['display_comment_count'] ) {
				?>
				<p>
			...
			if ( isset( $bdp_settings['display_postlike'] ) && 1 == $bdp_settings['display_postlike'] ) {
				?>
				<p><?php echo do_shortcode( '[likebtn_shortcode]' ); ?></p> 
				<?php
			}
			?>
		</div>
				...
				?>
					...
					if ( in_array( $post_type, $bdp_all_post_type ) ) {
							...
							if ( isset( $bdp_settings[ 'display_taxonomy_' . $taxonomy ] ) && 1 == $bdp_settings[ 'display_taxonomy_' . $taxonomy ] ) {
								...
								if ( isset( $taxonomy ) && ! in_array( $taxonomy, $bdp_exclude_taxonomy ) ) {
									if ( isset( $term_list ) && ! empty( $term_list ) ) {
										...
										?>
											<span class="post-category  <?php echo ( $taxonomy_link ) ? 'bdp-no-links' : 'bdp-has-links'; ?>">
						...
						if ( isset( $bdp_settings['display_category'] ) && 1 == $bdp_settings['display_category'] ) {
							...
							echo '<span class="post-category ' . esc_html( $categories_class ) . '"><i class="fas fa-folder"></i>';
							if ( $categories_link ) {
