...
if ( ! function_exists( 'bdp_single_boxy_template' ) ) {
	...
	function bdp_single_boxy_template( $bdp_settings ) {
		...
		<div class="blog_wrap blog_template bdp_blog_template boxy">
			...
			<?php
			$class = '';
			if ( ( isset( $bdp_settings['display_date'] ) && 1 == $bdp_settings['display_date'] ) || ( isset( $bdp_settings['display_author'] ) && 1 == $bdp_settings['display_author'] ) || ( isset( $bdp_settings['display_comment'] ) && 1 == $bdp_settings['display_comment'] ) ) {
				?>
				<div class="left_div">
				<?php
				if ( isset( $bdp_settings['display_author'] ) && 1 == $bdp_settings['display_author'] ) {
					?>
						<div class="author-avatar">
							...
							?>
							<label><?php esc_html_e( 'Author', 'blog-designer-pro' ); ?></label>
						</div>
						<?php
				}
						...
						<div class="post-meta">
							...
							?>
								...
								if ( isset( $bdp_settings['dsiplay_date_from'] ) && 'modify' === $bdp_settings['dsiplay_date_from'] ) {
									...
									esc_html_e( 'Posted Date', 'blog-designer-pro' );
								}
								?>
								</label>
							</div>
				...
				if ( isset( $bdp_settings['display_comment'] ) && 1 == $bdp_settings['display_comment'] ) {
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						?>
							...
							?>
								<label><?php esc_html_e( 'Comments', 'blog-designer-pro' ); ?></label>
							</span>
							<?php
					}
				}
