...
if ( isset( $_REQUEST['s'] ) && '' != $_REQUEST['s'] ) {
	...
	$where  = "WHERE single_download_name LIKE '%$search%'";
}
...
if ( isset( $_REQUEST['orderby'] ) && isset( $_REQUEST['order'] ) && 0 == $_REQUEST['order'] ) {
	$order_by    = 'desc';
	$ord         = 1;
	$order_field = 'single_download_name';
...
$total        = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(`id`) FROM ' . $wpdb->prefix . 'bdp_single_ed_download %s', $where ) );
...
$shortcodes = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bdp_single_ed_download %1s order by %1s %1s limit %d, %d", $where, $order_field, $order_by, $offset, $limit ) );
?>
<div class="bdp-admin wrap bdp-single-list">
	<?php
	if ( isset( $_GET['action'] ) && 'delete' === $_GET['action'] && isset( $_GET['id'] ) && ! empty( $_GET['id'] ) && isset( $_GET['page'] ) && 'single_edd_layouts' === $_GET['page'] && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) ) ) {
		?>
		<div class="updated">
			<p>
	...
	<h1>
		<?php esc_html_e( 'Single Easy Digital Product Layouts', 'blog-designer-pro' ); ?>
		<a class="page-title-action" href="?page=single_edd_download">
	...
	<form method="post">
		<ul class="subsubsub">
			<li class="all">
				<a class="current" href="?page=single_edd_layouts"><?php esc_html_e( 'All', 'blog-designer-pro' ); ?>
					<span class="count">(
					<?php
					if ( $total > 0 ) {
						echo esc_html( $total );
		...
		<div class="tablenav top">
			...
			>
				...
				<span class="pagination-links">
					...
					<?php } else { ?>
						<a class="first-page" href="<?php echo '?page=single_edd_layouts&paged=1&s=' . esc_attr( $search ); ?>" title="<?php esc_attr_e( 'Go to the first page', 'blog-designer-pro' ); ?>">&laquo;</a>
						<a class="prev-page" href="<?php echo '?page=single_edd_layouts&paged=' . esc_attr( $prev_page ) . '&s=' . esc_attr( $search ); ?>" title="<?php esc_attr_e( 'Go to the previous page', 'blog-designer-pro' ); ?>">&lsaquo;</a>
					<?php } ?>
					...
					<?php } else { ?>
						<a class="next-page " href="<?php echo '?page=single_edd_layouts&paged=' . esc_attr( $next_page ) . '&s=' . esc_attr( $search ); ?>" title="<?php esc_attr_e( 'Go to the next page', 'blog-designer-pro' ); ?>">&rsaquo;</a>
						<a class="last-page " href="<?php echo '?page=single_edd_layouts&paged=' . esc_attr( $num_of_pages ) . '&s=' . esc_attr( $search ); ?>" title="<?php esc_attr_e( 'Go to the last page', 'blog-designer-pro' ); ?>">&raquo;</a>
					<?php } ?>
				</span>
			</div>
		</div>
		<table id="username" class="wp-list-table widefat fixed striped bdp-sliders-list bdp-table bdp-sliders-list bdp-table">
			<thead>
				<tr>
					...
					<th class="manage-column column-single_download_name column-primary column-title sorted <?php echo esc_attr( $order_by ); ?>" scope="col" id="single_download_name">
						<a href="?page=single_edd_layouts&orderby=single_download_name&order=<?php echo esc_attr( $ord ); ?>">
			...
			<tbody id="the_list">
				...
				if ( ! $shortcodes ) {
					...
					esc_html_e( 'No Single Product Layout found.', 'blog-designer-pro' );
					echo '</td>';
					echo '</tr>';
				} else {
					$shortcode_cnt = 0;
					foreach ( $shortcodes as $shortcode ) {
						...
						$cat = '—';
						$tag = '—';
						if ( isset( $shortcode->single_download_template ) && 'tag' === $shortcode->single_download_template ) {
						...
						if ( isset( $shortcode->single_download_template ) && 'category' === $shortcode->single_download_template ) {
							if ( isset( $bdp_settings['template_category'] ) && ! empty( $bdp_settings['template_category'] ) ) {
								...
								foreach ( $categories as $t ) {
									$cat_name = get_term_by( 'id', $t, 'download_category' );
									$cat[]    = $cat_name->name;
								}
								$cat = join( ', ', $cat );
							}
						}
						$single_download_name = $shortcode->single_download_name;
						++$shortcode_cnt;
						echo '<tr>';
						?>
					...
					<td class="title column-title column-primary">
						<strong>
							<a href="<?php echo '?page=single_edd_download&action=edit&id=' . esc_attr( $shortcode->id ); ?>">
								<?php
								if ( ! empty( $single_download_name ) ) {
									echo esc_html( $single_download_name );
								} else {
						...
						<div class="row-actions">
							<span class="edit">
								<a title="<?php esc_attr_e( 'Edit this item', 'blog-designer-pro' ); ?>" href="<?php echo '?page=single_edd_download&action=edit&id=' . esc_attr( $shortcode->id ); ?>"><?php esc_html_e( 'Edit', 'blog-designer-pro' ); ?></a>
								|
							</span>
							<span class="duplicate">
								<a title="<?php esc_attr_e( 'Duplicate this item', 'blog-designer-pro' ); ?>" href="<?php echo esc_url( add_query_arg( 'action', 'duplicate_single_edd_download_in_edit', admin_url( 'admin.php?layout=' . $shortcode->id ) ) ); ?>"><?php esc_html_e( 'Duplicate', 'blog-designer-pro' ); ?></a>
								|
							</span>
							<span class="delete">
								<a title="<?php esc_attr_e( 'Delete this item', 'blog-designer-pro' ); ?>" href="<?php echo esc_url( wp_nonce_url( '?page=single_edd_layouts&action=delete&id=' . $shortcode->id ) ); ?>" onclick="return confirm('Do you want to delete this layout?');"><?php esc_html_e( 'Delete', 'blog-designer-pro' ); ?></a>
							</span>
						</div>
			...
			<tfoot>
				<tr>
					...
					<td class="manage-column column-single_download_name"><?php esc_html_e( 'Single Layout Name', 'blog-designer-pro' ); ?></td>
