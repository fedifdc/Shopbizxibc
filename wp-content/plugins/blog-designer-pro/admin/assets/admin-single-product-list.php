...
$total        = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(`id`) FROM ' . $wpdb->prefix . 'bdp_single_product %s', $where ) );
...
$shortcodes = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bdp_single_product %1s order by %1s %1s limit %d, %d", $where, $order_field, $order_by, $offset, $limit ) );
?>
<div class="bdp-admin wrap bdp-single-list">
	<?php
	if ( isset( $_GET['action'] ) && 'delete' === $_GET['action'] && isset( $_GET['id'] ) && ! empty( $_GET['id'] ) && isset( $_GET['page'] ) && 'single_product_layouts' === $_GET['page'] && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) ) ) {
		?>
		<div class="updated">
			<p>
	...
	<form method="post">
		...
		<div class="tablenav top">
			...
			>
				...
				<?php
				echo esc_html( number_format_i18n( $total ) ) . ' ' . sprintf( esc_attr( _n( 'item', 'items', $total, 'blog-designer-pro' ) ), esc_html( number_format_i18n( $total ) ) );
				?>
				</span>
					...
					<td class="title column-title column-primary">
						...
						<div class="row-actions">
							...
							<span class="duplicate">
								<a title="<?php esc_attr_e( 'Duplicate this item', 'blog-designer-pro' ); ?>" href="<?php echo esc_url( add_query_arg( 'action', 'duplicate_single_product_in_edit', admin_url( 'admin.php?layout=' . $shortcode->id ) ) ); ?>"><?php esc_html_e( 'Duplicate', 'blog-designer-pro' ); ?></a>
								|
							</span>
							<span class="delete">
								<a title="<?php esc_attr_e( 'Delete this item', 'blog-designer-pro' ); ?>" href="<?php echo esc_url( wp_nonce_url( '?page=single_product_layouts&action=delete&id=' . $shortcode->id ) ); ?>" onclick="return confirm('Do you want to delete this layout?');"><?php esc_html_e( 'Delete', 'blog-designer-pro' ); ?></a>
							</span>
						</div>
