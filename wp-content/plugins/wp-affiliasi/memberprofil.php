...
//$showtxt = $txterror = '';
// Cek apakah member punya data di wp_member
$getmember = $wpdb->get_row("SELECT * FROM `wp_member` WHERE `idwp`=".$user_ID);
...
// 		if (isset($_POST['ktp'])) { $updatedb .= "`ktp`= '".sanitize_text_field($_POST['ktp'])."',"; }
        if (isset($_POST['ktp'])) {
            $nik = sanitize_text_field($_POST['ktp']);
        
            // Check if NIK is exactly 16 characters
            if (strlen($nik) !== 16) {
                $txterror .= 'NIK harus terdiri dari 16 karakter.<br/>';
            }
        
            // Check if NIK is unique
            $check_nik = $wpdb->get_var($wpdb->prepare(
                "SELECT `id_user` FROM `wp_member` WHERE `ktp` = %s AND `idwp` != %d",
                $nik,
                $user_ID
            ));
        
            if ($check_nik > 0) {
                $txterror .= 'NIK sudah terdaftar. Harap gunakan NIK lain.<br/>';
            } else {
                $updatedb .= "`ktp`= '" . $nik . "',";
            }
        }
...
// 		if (isset($_POST['tgl_lahir'])) { $updatedb .= "`tgl_lahir`= '".sanitize_text_field($_POST['tgl_lahir'])."',"; }
        if (isset($_POST['tgl_lahir'])) {
            $tgl_lahir = sanitize_text_field($_POST['tgl_lahir']);
            $date = DateTime::createFromFormat('d/m/Y', $tgl_lahir);
            // Check if the date is valid and in the expected format
            if ($date && $date->format('d/m/Y') === $tgl_lahir) {
                $updatedb .= "`tgl_lahir`= '".$tgl_lahir."',";
            } else {
                // Handle invalid date error
                $txterror.= "Tanggal lahir tidak valid. Harap gunakan format dd/mm/yyyy.";
            }
        }
		...
		if (isset($_POST['email'])) { 
			...
			$check_email = $wpdb->get_var("SELECT `id_user` FROM `wp_member` WHERE `email` = '".$email."' AND `idwp` != ".$user_ID);
			...
			if (is_numeric($check_usermail) && $check_usermail != $user_ID) {
				$errmail = 1;
			}
		...
		if (isset($_POST['password']) && $_POST['password'] != '') { 
			...
			if ($password == $current_user->user_login) {
				$txterror .= 'Password tidak boleh sama dengan Username '.$current_user->user_login;
			} else {
		...
		if (isset($_POST['subdomain'])) { 
			...
			$subdomain_exist = $wpdb->get_var("SELECT `subdomain` FROM `wp_member` WHERE `subdomain`='$subdomain' && `idwp` != '$user_ID'");
		...
		if (isset($_FILES["gambar"]["name"]) && $_FILES["gambar"]["name"] != '') { 
			...
			if (!in_array($_FILES["gambar"]["type"], $whitelist_type)) {
			  ...
			  if ($_FILES["gambar"]["size"] > $max_size) {
				$txterror = 'Maaf, gambar terlalu besar. Max. 100kb';
				$uploadOk = 0;
			  }
	...
	if (isset($_FILES["foto_ktp"]["name"]) && $_FILES["foto_ktp"]["name"] != '') { 
			...
			$pic_dir = $upload_dir['basedir'].'/pic';			
			if( ! file_exists( $pic_dir ) ) { wp_mkdir_p( $pic_dir ); }
			$filename = 'user_ktp_'.$user_ID;
			$output_filename = $pic_dir.'/'.$filename;
			$target_file = $upload_dir['basedir'].'/pic/'.$filename;
            $uploadOk = 1;
            $imageFileType = strtolower(pathinfo($_FILES["foto_ktp"]["name"],PATHINFO_EXTENSION));
            ...
            //Check that the file is of the right type
			if (!in_array($_FILES["foto_ktp"]["type"], $whitelist_type)) {
			  $txterror = "Maaf, hanya support JPG, JPEG, PNG & GIF saja.";
			  $uploadOk = 0;
			}
			  ...
			  if ($_FILES["foto_ktp"]["size"] > $max_size) {
				$txterror = 'Maaf, gambar terlalu besar. Max. 10Mb';
				$uploadOk = 0;
			  }

			  if ($uploadOk == 1) {
				if (move_uploaded_file($_FILES["foto_ktp"]["tmp_name"], $target_file.'.'.$imageFileType)) {
// 				  $lainlain['pic_profil'] = $upload_dir['baseurl'].'/pic/'.$filename.'.'.$imageFileType;
				  $lainlain['foto_ktp'] = $upload_dir['baseurl'].'/pic/'.$filename.'.'.$imageFileType;
		...
		if(isset($lainlain['foto_ktp'])){
			$updatedb .= "`foto_ktp`='".$lainlain['foto_ktp']."',";
		}

		if ($txterror == '') {		
			$updatedb .= "`lastupdate`=NOW()";
			$wpdb->query("UPDATE wp_member SET ".$updatedb." WHERE `idwp`=".$user_ID." AND `id_user`=".$userid);
	...
	if (isset($_GET['page']) && $_GET['page'] == 'profil') {
		$showtxt .= '<h2>Edit Profile</h2>';
	}

	if (isset($txterror) && $txterror != '') {
		$showtxt .= '<div class="alert-danger"><strong>ERROR: '.$txterror.'</strong></div>';
	} 

	if (isset($sukses) && $sukses = 'ok') {
		$showtxt .= '<div class="alert-success"><strong>Profile Anda telah sukses diedit</strong></div>';
	}

	$aturform = get_option('aturform');
	$form = unserialize($aturform);
	$user = $wpdb->get_row("SELECT * FROM `wp_member` WHERE `idwp` = ".$user_ID);
	$custom = unserialize($user->homepage);
	$cc = 1;

	if (is_array($form)) {
		if (isset($custom['pic_profil']) && $custom['pic_profil'] != '') { 
			$pic = $custom['pic_profil']; 
		} else { 
			$pic = plugin_dir_url('kosong.png'); 
		}
		...
		if (isset($user->foto_ktp) && !empty($user->foto_ktp)) { 
			$pic = $user->foto_ktp; 
		} else { 
			$pic = plugin_dir_url('kosong.png'); 
		}
		$showtxt .= '<div class="cbform-row">
			<img id="foto_ktp" src="'.$pic.'?rand='.rand(0,999).'" alt="" style="width:100%;max-width:300px;margin:0 auto" />
		</div>
		<div class="cbform-row">			
			<label class="cbform-label">Foto KTP</label>
			<div class="cbform-field">				
				<input type="file" class="custom-file-input" name="foto_ktp" 
				title="Pilih Foto KTP" id="imgKtpInp" accept=".jpg, .gif, .jpeg, .png" />
				<br/><small>Max. 10Mb</small>
			</div>
		</div>';
		foreach ($form as $form) {
			if ($form['profil'] == 1) {
				...
				if (isset($form['info']) && $form['info'] != '') {
					$info = '<small>'.$form['info'].'</small></br></br>';
				} else {
					$info = '';
				}
				...
				// 			<div class="cbform-field"><input type="date" class="cbform-input" name="'.$name.'" value="'.$value.'"'.$required.'/>'.$info.'</div>
				// 		</div>';
				// 		break;
					case 'keterangan':
						$showtxt .= '<p class="cbform-info">'.$label.'</p>';
						break;
					case 'email':
						$showtxt .= '
						<div class="cbform-row">
					...
					case 'disable':
						...
						<div class="cbform-row">
							<label class="cbform-label">'.$label.'</label>
							<div class="cbform-field"><input type="text" class="cbform-input" name="'.$name.'" value="'.$value.'" disabled/></div>
						</div>';
						break;				
					default:
						$showtxt .= '
