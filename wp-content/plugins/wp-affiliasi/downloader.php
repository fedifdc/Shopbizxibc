...
$idproduk = $_GET['id'];
$produk = $wpdb->get_row("SELECT `membership`,`url_file` FROM `cb_produk` WHERE `id`=$idproduk");

if ($produk->membership == 2) {	
	$membership = $wpdb->get_var("SELECT `membership` FROM `wp_member` WHERE `idwp`=$user_ID");
	if ($membership == 2) {
		if(filter_var($produk->url_file, FILTER_VALIDATE_URL)) {
			header("Location:".$produk->url_file);
		} else {
			$file = $produk->url_file;
		}
	}
} elseif ($produk->membership == 3) {
...
if (!isset($filenew) || empty($filenew)) {
  $asfname = $fname;
}
else {
  // remove some bad chars
  $asfname = str_replace(array('"',"'",'\\','/'), '', $filenew);
