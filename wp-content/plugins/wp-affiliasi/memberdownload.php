...
if ($user_ID && isset($_GET['id']) && is_numeric($_GET['id'])) {
	...
	if ($member >= $membership) {
		...
		$showtxt .= '
		<p><a href="'.plugins_url('wp-affiliasi/downloader.php?id=').$idproduk.'"><img src="'.plugins_url('wp-affiliasi/download.gif').'" alt="Download" style="box-shadow:none;"/></a></p>';
	} else {
		if ($membership == 3) {
			...
			if ($cek == 1) {
				...
				$showtxt .= '				
		<p><a href="'.plugins_url('wp-affiliasi/downloader.php?id=').$idproduk.'"><img src="'.site_url().'/wp-content/plugins/wp-affiliasi/download.gif" alt="Download" border="0"></a></p>';
			} else {
