...
function member($atts) {
	...
	$a = shortcode_atts( 
		array(
			...
			'pesan' => 'Mohon info Lengkap bisnis Affiliate Shopbiz'
		), $atts);
...
function mysponsor($atts) {	
	...
	$a = shortcode_atts( 
		array(
			...
			'pesan' => 'Mohon info Lengkap tentang bisnis affiliate shopbiz '
		), $atts);
...
function cb_memberarea() {	
	...
	if (isset($user_ID) && is_numeric($user_ID) && $user_ID > 0) {
		...
		case 'withdraw' : include('memberwithdraw.php'); break;
		case 'order' : 
...
function premium($atts, $content = "") {
	...
	if (isset($user_ID) && $user_ID > 0) {
		if ($wpdb->get_var("SELECT `membership` FROM `wp_member` WHERE `idwp`=".$user_ID) == 2) {
			...
			return '<p>Konten ini hanya bisa dilihat oleh <b>Member Affiliator</b>, <a href="'.site_url().'/dasbor">Silahkan Upgrade dulu</a></p>';
		}
	} else {		
		return '<p>Konten ini hanya bisa dibaca oleh Member, <a href="'.wp_login_url(get_permalink()).'&reauth=1">silahkan login disini dulu</a> atau <a href="'.site_url().'/?page_id='.$options['registrasi'].'">Registrasi di sini</a></p>';
	}
}
		...
		foreach ($member as $key => $value) {	
			if ($key == 'homepage') {
				...
				foreach ($custom as $keyval => $customval) {
					$cari = '{'.$keyval.'}';
					$ganti = str_ireplace($cari, $customval, $ganti);
				}
			} else {
				$cari = '{'.$key.'}';
				$ganti = str_ireplace($cari, $value, $ganti);
			}
		}
		$ganti = str_ireplace('{pic_profil}', plugins_url('wp-affiliasi/img/nopic.jpg'), $ganti);
		$showtxt .= $ganti;
