...
if (isset($_GET['profil']) && is_numeric($_GET['profil'])) {
	if ($_GET['profil'] != 0) {
		...
		<tr>
		  <td>Update Profil</td>
		  <td>'.date('d-m-Y h:i:s',strtotime($user->lastupdate)).'</td>
		</tr>
		<tr>
		  <td>Status</td>
		  <td>';
		...
		<tr>
		  ...
		  <td>'.number_format($user->read).'</td>
		</tr>
		<tr>
		  <td>Jumlah Rekrut Langsung</td>
		  <td>'.number_format($wpdb->get_var("SELECT COUNT(*) FROM `wp_member` WHERE `id_referral`=".$user->idwp)).'</td>
		</tr>
		<tr>
		  <td>Total Pendapatan</td>
		  <td><a href="admin.php?page=cbaf_bayar&id='.$user->idwp.'">'.number_format($user->jml_voucher).'</a></td>
		</tr>
		<tr>
		  <td>Komisi belum dibayar</td>
		  <td>'.number_format($user->sisa_voucher).'</td>
		</tr>
		';
		}
		echo '
	...
	if (isset($cek->status) && $cek->status == 1) {
		...
		if (!isset($cek->id)) {
			...
			$wpdb->query("INSERT INTO `cb_produklain` (`idwp`,`id_user`,`idproduk`,`status`,`tgl_order`,`tgl_bayar`,`hargaproduk`) 
				VALUES (".$idwp.",".$id_user.",0,0,'".wp_date('Y-m-d H:i:s')."','".wp_date('Y-m-d H:i:s')."',".$options['harga'].")");
...
<table class="widefat">
	...
	if ($datausers) {
		...
		if ($view -> membership > 0) {	
			echo '<a href="admin.php?page=cbaf_memberlist&profil='.$view->idwp.'" target="_blank">'.$view -> nama.'</a>';
		} else {
			echo '<a href="admin.php?page=cbaf_memberlist&profil='.$view->id_user.'&val=0" target="_blank">'.$view -> nama.'</a>';
		}

		echo ' ('.$view -> username.')<br/>
			'.date('d-m-Y h:i',strtotime($view -> tgl_daftar)).'</td>
			<td><a href="admin.php?page=cbaf_memberlist&profil='.$view->id_referral.'" target="_blank">Lihat Sponsor</a></td>
		...
		echo '<a href="admin.php?page=cbaf_memberlist&hapus='.$view->idwp.'" onclick="javascript:return confirm(\'Anda yakin ingin menghapus \\\''.$view->nama.'\\\' ?\')">Hapus</a></td></tr>';
				}
			} else {
		echo '&nbsp;</td></tr>';
			}
