	...
	if (isset($data->status) && $data->status == 1) {
		...
		if (count($lapkeu) > 0) {
			...
			foreach ($lapkeu as $lap) {
				$komisi = (-1)*$lap->komisi;
				$ins .= "('".wp_date('Y-m-d H:i:s')."','Cancel ".esc_sql($lap->transaksi)."',".$lap->kredit.",".$komisi.",'refund',".$lap->id_user.",".$lap->id_sponsor.",".$id2up."),";
			...
			$wpdb->query("INSERT INTO `cb_laporan` 
				(`tanggal`,`transaksi`,`debet`,`komisi`,`keterangan`,`id_user`,`id_sponsor`,`id_order`) 
