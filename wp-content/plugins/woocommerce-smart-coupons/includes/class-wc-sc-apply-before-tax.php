...
if ( ! class_exists( 'WC_SC_Apply_Before_Tax' ) ) {
	...
	class WC_SC_Apply_Before_Tax {
		...
		private function __construct() {
			...
			add_filter( 'woocommerce_cart_coupon_types', array( $this, 'add_smart_coupon_valid_for_cart' ) );
		}
		...
		/**
		 * Adds the 'smart_coupon' type to the list of valid coupon types for the WooCommerce cart.
		 *
		 * This function appends 'smart_coupon' to the array of coupon types recognized by the WooCommerce cart.
		 * By hooking into the 'woocommerce_cart_coupon_types' filter, it ensures that 'smart_coupon' is treated as a valid coupon type when applying coupons to the cart.
		 *
		 * @param array $coupon_types The array of current valid coupon types.
		 * @return array Modified array of valid coupon types including 'smart_coupon'.
		 */
		public function add_smart_coupon_valid_for_cart( $coupon_types = array() ) {
