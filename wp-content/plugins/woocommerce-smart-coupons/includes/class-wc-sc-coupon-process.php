...
if ( ! class_exists( 'WC_SC_Coupon_Process' ) ) {
	...
	class WC_SC_Coupon_Process {
		...
		public function smart_coupons_contribution( $order_id = 0 ) {
			...
			if ( $order_has_smart_coupon ) {
				$is_apply_before_tax = get_option( 'woocommerce_smart_coupon_apply_before_tax', 'no' );
				// Add order meta to remember whether 'Apply before tax' is enabled or disabled when the order is created.
				$this->update_post_meta(
					$order_id,
					'wc_sc_environment',
					array(
						'sc_version'       => $this->get_version(),
						'apply_before_tax' => $is_apply_before_tax,
					),
					true,
					$order
				);
		...
		public function update_smart_coupon_balance( $order_id ) {
			...
			if ( ( 'shop_order' === $post_type && ! empty( $_POST['action'] ) && in_array( wc_clean( wp_unslash( $_POST['action'] ) ), array( 'edit_order', 'editpost' ), true ) && !in_array( $_POST['post_status'] , array( 'draft','trash' ) ) ) // phpcs:ignore
			...
			if ( ! empty( $order_used_coupons ) ) {
				...
				foreach ( $order_used_coupons as $coupon_item ) {
					if ( $coupon_item->get_meta( 'sc_coupon_deduction_done' ) !== 'yes' ) {
						$coupon_item->add_meta_data( 'sc_coupon_deduction_done', 'yes', true );
						$coupon_item->save();
		...
		public function modify_paypal_args( $args = array(), $order = null ) {
			...
			if ( 'yes' === $order_is_apply_before_tax ) {
				return $args;
			}
				...
				if ( $flag && 'add' === $operation ) {
					...
					if ( 'yes' === $is_send_email && 'yes' === $combine_emails ) {
						...
						if ( is_array( $coupon_receiver_details ) && ! empty( $coupon_receiver_details ) ) {
							 ...
							 *
							 * @param array $coupon_receiver_details Array of details for the combined coupon receiver.
							 ...
							 * }
							 *
							 * @return array $coupon_receiver_details Modified array of email to coupons details.
							 */
			...
			if ( ! empty( $coupons ) ) {
				...
				foreach ( $coupons as $item_id => $item ) {
					...
					if ( floatval( $discount ) > floatval( 0 ) ) {
						...
						// Add discount on tax if it has been given on tax.
						if ( 'no' === $order_is_apply_before_tax && ! empty( $discount_tax ) ) {
					...
					if ( $update ) {
						...
						$this->update_order_item_meta( $item_id, 'sc_refunded_discount', ( $sc_refunded_discount + $discount ), false, $order );
						$this->update_order_item_meta( $item_id, 'sc_refunded_discount_tax', ( $sc_refunded_discount_tax + $discount_tax ), false, $order );
