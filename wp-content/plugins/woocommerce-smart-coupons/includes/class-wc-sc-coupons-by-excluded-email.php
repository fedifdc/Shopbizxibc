...
if ( ! class_exists( 'WC_SC_Coupons_By_Excluded_Email' ) ) {
	...
	class WC_SC_Coupons_By_Excluded_Email {
		...
		public function validate( $valid = false, $coupon = object, $discounts = null ) {
			...
			$is_callable_coupon_emails_allowed = ( $this->is_wc_gte_90() ) ? $this->is_callable( 'Automattic\WooCommerce\Utilities\DiscountsUtil', 'is_coupon_emails_allowed' ) : $this->is_callable( $cart, 'is_coupon_emails_allowed' );
			...
			if ( true === $is_validate_allowed_emails && is_array( $customer_email ) && 0 < count( $customer_email ) && true === $is_callable_coupon_emails_allowed && ! $this->is_coupon_emails_allowed( $check_emails, $customer_email, $cart ) ) {
			...
			if ( is_array( $exclude_customer_email ) && 0 < count( $exclude_customer_email ) && true === $is_callable_coupon_emails_allowed && $this->is_coupon_emails_allowed( $check_emails, $exclude_customer_email, $cart ) ) {
				if ( true === $is_auto_applied ) {
					$valid = false;
					if ( $this->is_callable( $cart, 'has_discount' ) && $cart->has_discount( $coupon_code ) && $this->is_callable( $cart, 'remove_coupon' ) ) {
						if ( class_exists( 'WC_Coupon' ) && $this->is_callable( $coupon, 'add_coupon_message' ) ) {
