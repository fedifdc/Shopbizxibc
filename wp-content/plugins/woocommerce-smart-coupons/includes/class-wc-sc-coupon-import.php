	...
	class WC_SC_Coupon_Import extends WP_Importer {
		...
		public function process_bulk_generate_action() {
			...
			if ( empty( $post_smart_coupons_generate_action ) && empty( $post_generate_and_import ) ) {
				...
				$full_path          = $csv_file_data['wp_upload_dir'] . $csv_file_data['file_name'];
				$real_full_path     = realpath( $full_path );
				$allowed_mime_types = array( 'text/csv', 'text/comma-separated-values', 'application/csv', 'text/plain' );
				...
				if ( ! isset( $_POST['no_of_coupons_to_generate'] ) ) {
					...
					$fp         = array();
					$valid_file = true;
					if ( $real_full_path && strpos( wp_normalize_path( $real_full_path ), wp_normalize_path( realpath( $csv_file_data['wp_upload_dir'] ) ) ) === 0 && is_readable( $real_full_path ) ) {
						if ( function_exists( 'mime_content_type' ) ) {
							$file_mime_type = mime_content_type( $real_full_path );
							if ( in_array( $file_mime_type, $allowed_mime_types, true ) ) {
								$fp = file( $real_full_path );
							} else {
								$this->log( 'error', __( 'Only CSV files are supported. The uploaded file type is: ', 'woocommerce-smart-coupons' ) . $file_mime_type . ' ' . __FILE__ . ' ' . __LINE__ ); // phpcs:ignore
								$valid_file = false;
							}
						} else {
							$this->log( 'error', __( 'It seems PHP extension "fileinfo" is either not installed, not configured or it\'s not accessible.', 'woocommerce-smart-coupons' ) . ' ' . __FILE__ . ' ' . __LINE__ ); // phpcs:ignore
							$valid_file = false;
						}
					} else {
						$this->log( 'error', __( 'File not found. Make sure the file has required permission to read and/or the file is accessible: ', 'woocommerce-smart-coupons' ) . $real_full_path . ' ' . __FILE__ . ' ' . __LINE__ ); // phpcs:ignore
						$valid_file = false;
					}
				...
				$total_coupons_to_generate          = isset( $_POST['no_of_coupons_to_generate'] ) ? sanitize_text_field( wp_unslash( $_POST['no_of_coupons_to_generate'] ) ) : 0;
		...
		public function process_coupon( $post ) {
			...
			if ( 'yes' === $is_email_imported_coupons && ! empty( $customer_emails ) && ! is_scalar( $customer_emails ) && ( ! empty( $coupon_amount ) || 'yes' === $allowed_free_shipping || ( empty( $coupon_amount ) && ! empty( $coupon_actions ) ) ) && ! empty( $coupon_code ) && ! empty( $discount_type ) ) {
				$coupon       = array(
				...
				if ( ! empty( $posted_data ) && is_array( $posted_data ) ) {
					$message = ( ! empty( $posted_data['smart_coupon_message'] ) ) ? wpautop( do_shortcode( $posted_data['smart_coupon_message'] ) ) : '';
				}
