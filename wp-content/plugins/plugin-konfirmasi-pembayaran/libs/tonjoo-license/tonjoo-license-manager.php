				...
				if ( ! $license['active'] ) {
					if ( $is_expired && ! in_array( 'expired', $license['hide_notice'], true ) ) {
						if ( 'trial' === $license['type'] ) {
							$notices[ $name ]['expired'] = sprintf( __( 'Your trial for <strong>%s</strong> has ended.', 'tpc' ), esc_html( $plugin['Name'] ) );
						} else {
							$notices[ $name ]['expired'] = sprintf( __( 'Your license for <strong>%s</strong> has expired.', 'tpc' ), esc_html( $plugin['Name'] ) );
						}
					} elseif ( ! in_array( 'activate', $license['hide_notice'], true ) ) {
						$notices[ $name ]['activate'] = sprintf( __( 'Please activate your <strong>%s</strong> license.', 'tpc' ), esc_html( $plugin['Name'] ) );
					}
				} else {
					if ( 'trial' !== $license['type'] && ( $license['expiry'] - time() ) < MONTH_IN_SECONDS && ! in_array( 'month_to_expired', $license['hide_notice'], true ) ) {
						$notices[ $name ]['month_to_expired'] = sprintf( __( 'Your license for <strong>%s</strong> will ended soon.', 'tpc' ), esc_html( $plugin['Name'] ) );
					} elseif ( 'trial' === $license['type'] && ( $license['expiry'] - time() ) < ( 3 * DAY_IN_SECONDS ) && ! in_array( '3day_to_expired', $license['hide_notice'], true ) ) {
						$notices[ $name ]['3day_to_expired'] = sprintf( __( 'Your trial license for <strong>%s</strong> will ended soon.', 'tpc' ), esc_html( $plugin['Name'] ) );
					}
				}
			}
			$empty = true;
			...
			if ( ! $empty ) {
				...
				<div class="notice notice-warning is-dismissible">
					...
					<div class="buttons" style="margin: 10px 0;">
						<a href="<?php echo esc_url( admin_url( 'plugins.php?page=tonjoo_license_manager' ) ); ?>" class="button-primary"><?php esc_html_e( 'Manage License', 'tpc' ); ?></a>
						<a href="<?php echo esc_url( add_query_arg( array( 'hide' => $hide, 'ref' => $this->get_current_url() ), admin_url( 'plugins.php?page=tonjoo_license_manager' ) ) ); ?>" class="button"><?php esc_html_e( 'Hide Notice', 'tpc' ) ?></a>
					</div>
				</div>
				<?php
			}
				...
				if ( true === $force || true === $license['active'] ) {
					...
					if ( true === $force || $this->max_attempt > $license['check_attempt'] ) {
						...
						if ( true === $status['status'] ) {
							...
							if ( __( "Error: Can't connect to server", 'tpc' ) !== $status['data'] ) {
								$args = array(
									'key'             => $license['key'],
									'active'          => false,
									'type'            => $license['type'],
