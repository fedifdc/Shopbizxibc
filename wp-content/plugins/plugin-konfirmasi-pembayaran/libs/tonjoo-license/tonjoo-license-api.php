				...
				if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
					...
					if ( ! $license_status->status ) {
						...
						if ( 'failed activate, the license is already in use' === $license_status->message ) {
							return array(
								'status'    => false,
								'data'      => __( 'Error: The license is already in use. If you sure this license is belongs to you and no other sites are using this license, please contact us at the forum.', 'tpc' ),
							);
						} else {
							return array(
								'status'    => false,
					...
					return array(
						'status'    => false,
						'data'      => sprintf( __( 'Error: %s. Please contact us through the forum to resolve this.', 'tpc' ), wp_remote_retrieve_response_code( $request ) . ' ' . wp_remote_retrieve_response_message( $request ) ),
					);
				}
			...
			if ( is_wp_error( $request ) ) {
				...
				if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
					...
					if ( isset( $response->error ) && $response->error ) {
						...
						return array(
							'status'    => false,
							'data'      => sprintf( __( 'Error: %s. Please try again.', 'tpc' ), $response->message ),
						);
					}
