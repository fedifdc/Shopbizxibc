...
if (isset($_GET['import']) && is_numeric($_GET['import'])) {
	$grup = $wpdb->get_row("SELECT * FROM `wafu_grup` WHERE `grup_id`=".$_GET['import']);
  ...
  $i = 0;
	if ( $members = SimpleXLSX::parse($targetPath) ) {
	    foreach ($members->rows() as $member) {	    		
	  		$nowa = wafu_format($member[($_POST['nowa'])]);
	  		if ($nowa != '' && strlen($nowa) > 8) {
	    		...
	    		$insert_member .= "(".$grup->grup_id.",'".$wpdb->_real_escape($member[($_POST['nama'])])."','".$nowa."','".$dbcustom."'),";
	    		$i++;
	  		}
	  	}

	  	if ($insert_member != '') {
		  	$wpdb->query("INSERT INTO `wafu_import` (`import_grup`,`import_nama`,`import_wa`,`import_custom`) 
...
?> 
	...
	if (isset($_GET['del']) && is_numeric($_GET['del'])) {
		$wpdb->query("DELETE FROM `wafu_import` WHERE `import_id`=".$_GET['del']);
	...
	$data = $wpdb->get_results("SELECT * FROM `wafu_import` 
		LEFT JOIN `wafu_grup` ON `wafu_grup`.`grup_id` = `wafu_import`.`import_grup`
		LIMIT ".$start.",".$perpage);
	if (count($data) > 0) {
		echo '
...
<div class="table-responsive">
	...
	<tbody>';
		...
		<td><a href="admin.php?page=wafu_member&import='.$_GET['import'].'&del='.$data->import_id.'" class="btn btn-sm btn-danger">Delete</a></td>
			';
		}
		echo '
	</tbody>
...
$jml = $wpdb->get_var("SELECT COUNT(*) FROM `wafu_import`"); 
