...
if (isset($_GET['campaign']) && is_numeric($_GET['campaign'])) :
	$grupaktif = $wpdb->get_row("SELECT * FROM `wafu_grup` WHERE `grup_id`=".$_GET['campaign']);
	echo '
	<div class="wrap">
    ...
    if (isset($txterror) && $txterror != '') {
		...
		if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
			if (isset($_POST['camp_title']) && isset($_POST['camp_grup'])) {
				...
				$wpdb->query("UPDATE `wafu_campaign` SET 
				...
				if ($wpdb->last_error !== '') {
					...
					<p>Update berhasil. <a href="admin.php?page=wafu_grup&campaign='.$_GET['campaign'].'">Kembali ke campaign</a></p>
					</div>';
				}
			}
			$data = $wpdb->get_row("SELECT * FROM `wafu_campaign` WHERE `camp_id`=".$_GET['edit']);
		} else {
			if (isset($_POST['camp_title']) && isset($_POST['camp_grup'])) {
				if (isset($gambar) && $gambar != '') { 
					# Diam aja
				...
				$wpdb->query("INSERT INTO `wafu_campaign` 
					(`camp_grup`,`camp_title`,`camp_content`,`camp_image`,`camp_delay`,`camp_periode`,`camp_sort`) VALUES 
					(
				...
				foreach ($_POST['camp'] as $sort) {
					$wpdb->query("UPDATE `wafu_campaign` SET `camp_sort`=".$sort['sort']." WHERE `camp_id`=".$sort['id']);
				}
				echo '<div class="notice notice-success is-dismissible">
					<p>Campaign telah diurutkan</p>
					</div>';
			} elseif (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
				$wpdb->query("DELETE FROM `wafu_campaign` WHERE `camp_id`=".$_GET['hapus']);
			} 		

			$camp = $wpdb->get_results("SELECT * FROM `wafu_campaign` WHERE `camp_grup`=".$_GET['campaign']." ORDER BY `camp_sort`");
			$i = 1;
			if (count($camp) > 0) {
				echo '
			...
			';
				...
				<td><a href="admin.php?page=wafu_grup&campaign='.$_GET['campaign'].'&edit='.$camp->camp_id.'" class="btn btn-success btn-sm">Edit</a> &nbsp;
				<a href="admin.php?page=wafu_grup&campaign='.$_GET['campaign'].'&hapus='.$camp->camp_id.'" class="btn btn-danger btn-sm">Hapus</a></td>
				</tr>';
					$i++;
				}
				echo '</tbody>
