...
<a href="admin.php?page=wafu_bc#newbc" class="page-title-action">Add New</a>
<hr class="wp-header-end">
<?php
$grup = $wpdb->get_results("SELECT `grup_id`,`grup_nama` FROM `wafu_grup`");
if (count($grup) > 0) {
	...
	if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
		...
		$data = $wpdb->get_row("SELECT * FROM `wafu_bc` WHERE `bc_id`=".$_GET['edit']);
		...
		$data = $wpdb->get_row("SELECT * FROM `wafu_bc` WHERE `bc_id`=".$_GET['resend']);
		$datatarget = unserialize($data->bc_target);
		$act = 'Resend';
	} elseif (isset($_GET['cancel']) && is_numeric($_GET['cancel'])) {
		$wpdb->query("UPDATE `wafu_bc` SET `bc_status`=2 WHERE `bc_id`=".$_GET['cancel']);
		$wpdb->query("UPDATE `wafu_gruptomember` SET `gtm_bc`=0");
		...
		$wpdb->query("DELETE FROM `wafu_bc`  WHERE `bc_id`=".$_GET['hapus']);
		echo '<div class="notice notice-success is-dismissible">
		...
		if (isset($_POST['bc_judul']) && isset($_POST['bc_isi'])) {
			...
			$wpdb->query("INSERT INTO `wafu_bc` 
				(`bc_judul`,`bc_isi`,`bc_image`,`bc_tglkirim`,`bc_status`,`bc_target`,`khususdone`) 
				VALUES
				(
	...
	$databc = $wpdb->get_results("SELECT * FROM `wafu_bc` ORDER BY `bc_status`,`bc_tglkirim` DESC");
	if (count($databc) > 0) {
		...
		foreach ($databc as $bc) {
			...
			if ($bc->bc_status == 2) {
				echo '<td>Selesai</td>
				<td><a href="admin.php?page=wafu_bc&resend='.$bc->bc_id.'#newbc" class="btn btn-sm btn-success">Resend</a>';
			} elseif ($bc->bc_status == 1) {
				echo '<td class="text-success">Running</td>
				<td><a href="admin.php?page=wafu_bc&cancel='.$bc->bc_id.'" class="btn btn-sm btn-warning">Cancel</a>';
			} else {
				echo '<td>Antri</td><td>';
			}
			echo '
			<a href="admin.php?page=wafu_bc&edit='.$bc->bc_id.'#newbc" class="btn btn-sm btn-primary">Edit</a> &nbsp;
			<a href="admin.php?page=wafu_bc&hapus='.$bc->bc_id.'" class="btn btn-sm btn-danger">Hapus</a>
			</td>
			</tr>';
		}
		echo '
		...
		<div class="form-group row">
	        ...
	        <div class="col-sm-10">
	            ...
	            if (isset($data->bc_image) && $data->bc_image != '') {
	            	...
	            	echo plugins_url('wafu/kosong.png');
	            }
	            ?>" alt="" class="img-fluid" />
	          	</div>
	        </div>
...
<?php	
	...
	echo '<p><a href="admin.php?page=wafu_grup">Klik di sini</a> untuk membuat grup terlebih dahulu</p>';
}
?>
<p>&nbsp;</p>
<div id="show"></div>
