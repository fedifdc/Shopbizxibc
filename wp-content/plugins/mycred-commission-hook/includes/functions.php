...
add_action( 'mycred_load_hooks', 'mycred_load_commission_hook' );

function mycred_load_commission_hook() {
    if ( ! class_exists( 'MyCred_Commission_Hooks' ) ) :

        class MyCred_Commission_Hooks extends myCRED_Hook {
            ...
            public function run() {
                ...
                // add_action('woocommerce_order_status_completed', array($this, 'award_commission_points'), 10, 1);
                add_action('woocommerce_order_status_completed', array($this, 'schedule_award_commission_points'), 10, 1);

                add_action('background_award_commission_points', array($this, 'award_commission_points'), 10, 1);

                // add_action('woocommerce_payment_complete', array($this, 'award_commission_points'), 10, 1);
                ...
                add_action('revoke_sponsor_commission_cron', [$this,'revoke_sponsor_commission_points']);
                $this->run_revoke_hook();

                // add_action( 'woocommerce_thankyou', array($this, 'award_commission_points'), 10, 1);
            }
	        ...
	        public function schedule_award_commission_points($order_id) {
                if (!wp_next_scheduled('background_award_commission_points', array($order_id))) {
                    wp_schedule_single_event(time() + 10, 'background_award_commission_points', array($order_id));
                }
            }
            ...
            public function award_commission_points( $order_id ) {
                error_log("Starting award_commission_points for order_id: $order_id");
                ...
                error_log("Finished award_commission_points for order_id: $order_id");
            }
          
          	
			
			public function revoke_sponsor_commission_points($order_id) {
                error_log("Starting revoke_sponsor_commission_points for order: " . $order_id);
                ...
                error_log("Completed revoke_sponsor_commission_points for order: " . $order_id);
            }
            ...
            private function calculate_commission( $sponsor_id, $points ) {
                ...
                $commission_percent = isset( $this->prefs['commission_percent'][ $badge_level ] ) 
                    ? (float) $this->prefs['commission_percent'][ $badge_level ]
                    : 0;
                ...
                error_log("MYCRED COMMISION HOOK - Points: $points, Commission Percent: $commission_percent");
                error_log( "Points: " . var_export($points, true) );
                error_log( "Commission Percent: " . var_export($commission_percent, true) );
                return ( $points * $commission_percent ) / 100;
            }
            */
			private function calculate_commission($sponsor_id, $points) {
                $badge_level = $this->get_badge_level($sponsor_id);
                $commission_percent = $this->prefs['commission_percent'][$badge_level] ?? 0;
                return ($points * $commission_percent) / 100;
            }
          ...
          private function get_display_name($user_id) {
			...
			public function preferences() {
				...
				$prefs = $this->prefs;
				?>
				<label for="commission_percent[Free Member]"><?php _e( 'Free Member Badge Commission (%)', 'mycred' ); ?></label>
				<input type="number" name="<?php echo $this->field_name( array( 'commission_percent', 'Free Member' ) ); ?>" id="commission_percent[Free Member]" value="<?php echo $prefs['commission_percent']['Free Member']; ?>" class="small-text" />%
				<br>
				<label for="commission_percent[Starter]"><?php _e( 'Starter Badge Commission (%)', 'mycred' ); ?></label>
				<input type="number" name="<?php echo $this->field_name( array( 'commission_percent', 'Starter' ) ); ?>" id="commission_percent[Starter]" value="<?php echo $prefs['commission_percent']['Starter']; ?>" class="small-text" />%
				<br>
				<label for="commission_percent[Basic]"><?php _e( 'Basic Badge Commission (%)', 'mycred' ); ?></label>
				<input type="number" name="<?php echo $this->field_name( array( 'commission_percent', 'Basic' ) ); ?>" id="commission_percent[Basic]" value="<?php echo $prefs['commission_percent']['Basic']; ?>" class="small-text" />%
				<br>
				<label for="commission_percent[Premium]"><?php _e( 'Premium Badge Commission (%)', 'mycred' ); ?></label>
				<input type="number" name="<?php echo $this->field_name( array( 'commission_percent', 'Premium' ) ); ?>" id="commission_percent[Premium]" value="<?php echo $prefs['commission_percent']['Premium']; ?>" class="small-text" />%
				<br>
				<label for="commission_percent[Prestige]"><?php _e( 'Prestige Badge Commission (%)', 'mycred' ); ?></label>
				<input type="number" name="<?php echo $this->field_name( array( 'commission_percent', 'Prestige' ) ); ?>" id="commission_percent[Prestige]" value="<?php echo $prefs['commission_percent']['Prestige']; ?>" class="small-text" />%
				<?php
			}
			...
			public function sanitise_preferences( $data ) {
				...
				$new_data = $data;
				$new_data['commission_percent']['Starter']  = absint( $data['commission_percent']['Starter'] );
				$new_data['commission_percent']['Basic']    = absint( $data['commission_percent']['Basic'] );
				$new_data['commission_percent']['Premium']  = absint( $data['commission_percent']['Premium'] );
				$new_data['commission_percent']['Prestige'] = absint( $data['commission_percent']['Prestige'] );
              	$new_data['commission_percent']['Free Member'] = absint( $data['commission_percent']['Free Member'] );
				return $new_data;
			}
        }
    endif;
