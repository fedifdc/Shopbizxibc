...
function woocommerce_xendit_pg_init()
{
    ...
    function xendit_woo_redirect_invoice($str, $order)
    {
        ...
        if (!empty($invoice_url)) {
            ?>
            <div id="xendit-invoice-wrapper">
                <div class="loading-ring">
                    <div></div><div></div><div></div><div></div>
                </div>
                <p id="xendit-invoice-countdown" class="loading-text"></p>
            </div>
                ...
                window.addEventListener("load", function () {
                    var x = setInterval(function () {
                        if (timeLeft == 0) {
                            ...
                            elem.classList.remove('loading-text');
                        } else {
                            elem.innerHTML = 'Processing your order... Redirecting in ' + timeLeft + ' seconds';
                            timeLeft--;
                        }
                    }, 1000);
                });
            ...
            <style>
                #xendit-invoice-wrapper {
                    text-align: center;
                    margin-top: 50px;
                }
                /* Spinner besar ala ring loading */
                .loading-ring {
                ...
                .loading-ring div {
                    ...
                    height: 64px;
                    margin: 8px;
                    border: 8px solid #4481F1;
                    border-radius: 50%;
                    animation: loading-ring-spin 1.2s cubic-bezier(0.5, 0, 0.5, 1) infinite;
                    border-color: #4481F1 transparent transparent transparent;
                }
