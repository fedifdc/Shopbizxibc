     ...
     */
        ...
        <div class="store-cat-stack-dokan cat-drop-stack">
            ...
            if ( $vendor instanceof Vendor ) {
                ...
                // print("</pre>");
                // die(0);
                echo '<ul class="store-category-list desktop-only">'; // Tambahkan class
                echo call_user_func_array(
                    [ &$walker, 'walk' ],
                    [ $categories, 0, ['style' => 'list'] ]
                ); //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
                echo '</ul>';
                //mobile
                // Mobile Dropdown
                // echo '<select class="store-category-list mobile-only" onchange="location.href=this.value">';
                // echo '<option value="">Pilih Kategori</option>';
                // foreach ( $categories as $category ) {
                   
                //     if ( isset( $category->term_id ) ) {
                //         echo '<option value="' . esc_url( get_term_link( $category->term_id, 'product_cat' ) ) . '">' . esc_html( $category->name ) . '</option>';
                //     }
                // }
                // echo '</select>';
            }
