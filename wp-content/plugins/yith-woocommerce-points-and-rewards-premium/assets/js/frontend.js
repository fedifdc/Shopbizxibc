	...
	var $body = $('body'),
		apply_points = function ($form) {
			...
			$.ajax({
				...
				//url: yith_ywpar_general.wc_ajax_url.toString().replace('%%endpoint%%', 'apply_wc_points_rewards'),
				data: data,
				dataType: 'html',
				success: function (response) {
					console.log('ywpar_applied tes');
				},
				complete: function () {
	...
	$.fn.yith_ywpar_variations = function ($form) {
		...
		$form.on('found_variation', function (event, variation) {
			...
			if (variation.variation_points) {
				$points.text(Math.floor(variation.variation_points));
			}
