...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Frontend' ) ) {
	...
	class YITH_WC_Points_Rewards_Frontend extends YITH_WC_Points_Rewards_Frontend_Legacy {
		...
		private function get_cart_message( $total_points = 0 ) {
			...
			if ( 'fixed' === $conversion_method ) {
				...
				//$discount    = custom_max_points_redeem();
			}
		...
		private function get_rewards_message() {
			...
			$args     = array(
				...
				'max_discount'                  => custom_max_points_redeem(),//$max_discount,
				'max_percentage_discount'       => yith_points()->redeeming->get_max_percentage_discount(),
				'max_points'                    => custom_max_points_redeem(),//$max_points,
	...
	function custom_max_points_redeem() {
		...
		foreach ( $cart_items as $cart_item ) {
			...
			if ( $product->is_type( 'simple' ) ) {
				...
				$redeem_point =get_post_meta( $product_id, '_redeem_points', true );
				$maksimal_tukar = ($redeem_point != null) ? $redeem_point : 0;
				...
				if ( $variation_id ) {
					...
					$redeem_point = get_post_meta( $variation_id, '_variation_redeem_points', true );
					$maksimal_tukar = ($redeem_point != null) ? $redeem_point : 0;
				}
			} else {
