...
// print("<pre>");
// 			print_r($this->max_discount);
// 			print_r($this->max_points );
// 			print("</pre>");
// 			die(0);
...
/*
		...
		public function apply_discount_calculation( $posted, $apply_coupon = true ) {
			...
			if ( 'fixed' === $reward_method ) {
				...
				$conversion              = $this->get_conversion_rate_rewards();
				$input_max_discount      = ($input_points < product_max_points_redeem() ? $input_points : product_max_points_redeem() );//edit ruli
				$input_points            = $input_max_discount;
	...
	function product_max_points_redeem() {
    $cart_items = WC()->cart->get_cart();
    $custom_max_points = 0; // Initialize the custom max points

    foreach ( $cart_items as $cart_item ) {
        $product_id = $cart_item['product_id'];
        $quantity = $cart_item['quantity']; // Get the quantity of the item in the cart
        $product = wc_get_product( $product_id ); // Get the product object
        $maksimal_tukar = 0; // Initialize maksimal_tukar

        if ( $product->is_type( 'simple' ) ) {
            // Handle simple product
            $maksimal_tukar = get_post_meta( $product_id, '_redeem_points', true );

        } elseif ( $product->is_type( 'variable' ) || $product->is_type( 'variation' ) ) {
            // Handle variable or variation product
            $variation_id = $cart_item['variation_id']; // The variation ID, if any

            if ( $variation_id ) {
                $variation = wc_get_product( $variation_id ); // Get the variation object
                $maksimal_tukar = get_post_meta( $variation_id, '_variation_redeem_points', true );
            }
        } else {
            // Handle other product types, if any (grouped, bundled, etc.)
        }

        // Use the maximum value from the 'maksimal_tukar' field if it exists
        if ( ! empty( $maksimal_tukar ) ) {
            // Multiply maksimal_tukar by the quantity of the item
            $custom_max_points += (int) $maksimal_tukar * $quantity;
        }
    }

    // Return the custom maximum points for redemption
