		...
		<div class="license-wrapper">
			<div class="license-content">
					...
					<div class="meta-box">
						<div class="postbox">
							...
							<div class="inside" style="z-index:1;">
								...
								<table class="form">
									<tr>
										<td class="title"><?php esc_html_e( 'License Status', 'pokmv' ); ?></td>
										<td class="field">
											<?php if ( $is_active ) : ?>
												<p class="license-status stat-success">
													<span class="dashicons dashicons-yes"></span>
													<?php if ( 'trial' === $plugin['type'] ) : ?>
														<span><?php esc_html_e( 'Trial Activated', 'pokmv' ); ?></span>
													<?php else : ?>
														<span><?php esc_html_e( 'Activated', 'pokmv' ); ?></span>
													<?php endif; ?>
												</p>
											<?php else : ?>
												<?php if ( $is_expired ) : ?>
													<p class="license-status stat-error">
														...
														<?php if ( 'trial' === $plugin['type'] ) : ?>
															<span><?php esc_html_e( 'Trial Expired', 'pokmv' ); ?></span>
														<?php else : ?>
															<span><?php esc_html_e( 'Expired', 'pokmv' ); ?></span>
														<?php endif; ?>
													</p>
												<?php else : ?>
													<p class="license-status stat-error">
														<span class="dashicons dashicons-no-alt"></span>
														<span><?php esc_html_e( 'Not Activated', 'pokmv' ); ?></span>
													</p>
												<?php endif; ?>
											<?php endif; ?>
										</td>
									...
									<tr>
										<td class="title"><?php esc_html_e( 'Your License Code', 'pokmv' ); ?></td>
										<td class="field">
											<div class="license-field">
												<div class="field">
													...
													<?php if ( $is_active ) : ?>
														<button data-context="deactivate" class="submit-key button-primary"><?php esc_html_e( 'Deactivate', 'pokmv' ); ?></button>
													<?php else : ?>
														<button data-context="activate" class="submit-key button-primary"><?php echo $is_expired ? __( 'Renew License', 'pokmv' ) : __( 'Check License', 'pokmv' ); ?></button>
													<?php endif; ?>
												</div>
												<div class="status <?php echo $is_active ? 'stat-success' : 'stat-error'; ?>">
													<?php
													if ( $is_active ) {
														printf( __( 'Valid until: %s', 'pokmv' ), $this->get_local_time( $plugin['expiry'] ) );
														if ( ( $plugin['expiry'] - time() ) < WEEK_IN_SECONDS && 'trial' !== $plugin['type'] ) {
															echo ' ';
															printf( __( '(expired soon, <a href="%s" target="_blank">click to renew</a>)', 'pokmv' ), 'https://tonjoostudio.com/manage/user/myItem/' );
														} elseif ( ( $plugin['expiry'] - time() ) < DAY_IN_SECONDS && 'trial' === $plugin['type'] ) {
															echo ' ';
															printf( __( '(expired soon, <a href="%s" target="_blank">click to renew</a>)', 'pokmv' ), 'https://tonjoostudio.com/manage/user/myItem/' );
														}
													} elseif ( $is_expired ) {
														printf( __( 'Expired at: %s', 'pokmv' ), $this->get_local_time( $plugin['expiry'] ) );
													} else {
														esc_html_e( 'Input your license key', 'pokmv' );
													}
													?>
												</div>
											</div>
									...
									<tr>
										<td class="title"><?php esc_html_e( 'Last Check' ,'pokmv' ); ?></td>
				...
				<div class="upsell-separator">
					<h3><?php esc_html_e( 'Have you tried our other cool stuffs?', 'pokmv' ) ?></h3>
				</div>
				<?php
			...
			<div class="license-sidebar">
				<div class="meta-box">
					<div class="postbox">
						<h3 class="hndle"><span class="dashicons dashicons-sos"></span> <?php echo esc_html_e( 'Help Center', 'pokmv' ); ?></h3>
						<div class="item">
							<p><?php esc_html_e( 'Where can I get my licenses?', 'pokmv' ) ?></p>
							<a href="https://tonjoostudio.com/account/items/" class="button" target="_blank"><?php echo esc_html_e( 'My Licenses', 'pokmv' ) ?></a>
						</div>
						<div class="item">
							<p><?php esc_html_e( 'I have a problem with my license', 'pokmv' ) ?></p>
							<a href="https://forum.tonjoostudio.com/" class="button" target="_blank"><?php echo esc_html_e( 'Support Forum', 'pokmv' ) ?></a>
						</div>
					</div>
				</div>
			</div>
