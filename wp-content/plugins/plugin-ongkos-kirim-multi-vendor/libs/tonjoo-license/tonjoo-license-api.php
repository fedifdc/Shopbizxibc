	...
	class Tonjoo_License_API {
		...
		public function activate( $key = '' ) {
			if ( empty( $key ) ) {
				return array(
					'status'    => false,
					'data'      => __( 'Error: License key is empty', 'pokmv' ),
				);
			}
			...
			if ( is_wp_error( $request ) ) {
				...
				return array(
					'status'    => false,
					'data'      => __( "Can't connect to server. Please try again.", 'pokmv' ),
				);
			} else {
				if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
					...
					if ( ! $license_status->status ) {
						...
						if ( 'failed activate, the license is already in use' === $license_status->message ) {
							return array(
								'status'    => false,
								'data'      => __( 'Error: The license is already in use. If you sure this license is belongs to you and no other sites are using this license, please contact us at the forum.', 'pokmv' ),
							);
						} else {
							return array(
								'status'    => false,
								'data'      => sprintf( __( 'Error: %s. Please try again.', 'pokmv' ), $license_status->message ),
							);
						}
					}
				} else {
					...
					return array(
						'status'    => false,
						'data'      => sprintf( __( 'Error: %s. Please contact us through the forum to resolve this.', 'pokmv' ), wp_remote_retrieve_response_code( $request ) . ' ' . wp_remote_retrieve_response_message( $request ) ),
					);
				}
				...
				if ( is_wp_error( $request ) ) {
					...
					return array(
						'status'    => false,
						'data'      => sprintf( __( 'Error: %s. Please try again.', 'pokmv' ), $request->get_error_message() ),
					);
				} else {
					if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
						// write json file.
		...
		public function status( $key = '', $debug = false ) {
			...
			if ( is_wp_error( $request ) ) {
				...
				return array(
					'status'    => false,
					'data'      => __( "Error: Can't connect to server", 'pokmv' ),
				);
			} else {
				if ( 200 === wp_remote_retrieve_response_code( $request ) ) {
					...
					if ( isset( $response->error ) && $response->error ) {
						...
						return array(
							'status'    => false,
							'data'      => sprintf( __( 'Error: %s. Please try again.', 'pokmv' ), $response->message ),
						);
					}
