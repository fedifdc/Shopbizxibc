							...
							<?php foreach ( $services as $courier => $courier_services ) : ?>
								<div class="options-specific-service-<?php echo esc_attr( $courier ); ?> service-options">
									...
									<div class="courier-service-options">
										...
										foreach ( $courier_services as $key => $service ) {
											?>
											<input type="checkbox" value="<?php echo esc_attr( $courier . '-' . $key ); ?>" data-short="<?php echo esc_attr( $service['short'] ); ?>" name="pokmv_vendor[specific_service_option][]" id="setting-service-<?php echo esc_attr( $courier . '-' . $key ); ?>" <?php echo in_array( $courier . '-' . $key, $vendor_settings['specific_service_option'], true ) ? 'checked' : ''; ?>>
