...
function sb_get_front_settings() {
    ...
    if (($return['registration_required'] == 'registration-login' || $return['registration_required'] == 'login') && !sb_isset(sb_get_setting('registration-fields'), 'reg-email')) {
        $return['registration_required'] = $return['registration_required'] == 'login' ? false : 'registration';
    }
    if ($return['rating']) {
...
function sb_get_articles($article_id = false, $count = false, $full = false, $categories = false, $language = false, $skip_language = false) {
    ...
    $is_multilingual_by_translation = $language && $language != 'all' && (1 || sb_get_multi_setting('google', 'google-multilingual-translation')); //temp
