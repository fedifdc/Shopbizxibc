    ...
    if (!empty($email)) {
        ...
        if ($existing_user) {
            if (empty($settings['otp'])) {
                ...
                return sb_login(false, false, $existing_user['id'], $existing_user['token']);
            }
        }
    }
...
function sb_routing_and_department_db($table_name = 'sb_conversations', $users = false) {
    ...
    $routing = sb_isset(sb_get_active_user(), 'user_type') == 'agent' && (sb_get_multi_setting('queue', 'queue-active') || sb_get_multi_setting('routing', 'routing-active') || $hide);
