...
<form class="woocommerce-shipping-calculator" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<section class="shipping-calculator-form" style="display:none;">
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_country', true ) ) :
			...
			<?php else: ?>
				<input type="hidden" name="calc_shipping_country" id="calc_shipping_country" value="<?php echo esc_attr( WC()->customer->get_shipping_country() ); ?>">
			<?php endif; ?>
		<?php endif; ?>
			...
			<p class="form-row form-row-wide" id="calc_shipping_state_field">
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					<span>
						...
						<select name="calc_shipping_state" class="state_select" id="billing_state" data-placeholder="<?php esc_attr_e( 'State / County', 'woocommerce' ); ?>"  data-selected="<?php echo esc_attr( WC()->customer->get_shipping_state() ); ?>">
						...
						<input type="hidden" name="cl_bill_state_name" id="cl_bill_state_name">
					</span>
					<?php
				} else {
					?>
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_city', true ) ) : ?>
			<p class="form-row form-row-wide" id="calc_shipping_city_field">
				...
				<select name="calc_shipping_city" class="calc_shipping_city select2" id="billing_city" data-placeholder="<?php esc_attr_e( 'City', 'woocommerce' ); ?>" data-selected="<?php echo esc_attr( WC()->customer->get_billing_city() ); ?>">
					<option value=""><?php esc_html_e( 'Select an option&hellip;', 'woocommerce' ); ?></option>
					?>
				</select>
				<input type="hidden" name="cl_bill_city_name" id="cl_bill_city_name">
			</p>
		<?php endif; ?>

		<?php if ( get_option( 'woocommerce_type_account') == 'pro' || get_option('cl_ongkir_choose_key') != 'custom' ) :
			global $current_user;
			$user_id = $current_user->data->ID;
			$index_city =  get_user_meta( $user_id, 'billing_subdistrict', true );
		?>
			<p class="form-row form-row-wide" id="calc_shipping_subdistrict_field">
				<label for="calc_shipping_subdistrict" class="screen-reader-text"><?php esc_html_e( 'Subdistrict', 'woocommerce' ); ?></label>
				<select name="calc_shipping_subdistrict" class="calc_shipping_subdistrict select2" id="billing_subdistrict" data-placeholder="<?php esc_attr_e( 'Subdistrict', 'woocommerce' ); ?>" data-selected="<?php echo esc_attr( $index_city ); ?>">
					<option value=""><?php esc_html_e( 'Select an option&hellip;', 'woocommerce' ); ?></option>
					?>
				</select>
				<input type="hidden" name="cl_bill_subdistrict_name" id="cl_bill_subdistrict_name">
			</p>
		<?php endif; ?>
